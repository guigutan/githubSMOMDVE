...
Ext.define('SIE.Domain.State', {
    statics: {
        Enable: { value: 1, text: 'Enable', label: '可用'.t() },
        Disable: { value: 0, text: 'Disable', label: '禁用'.t() }
    }
});
    ...
    loadChildData: function (opType) {
        ...
        if (parent) {
            ...
            if (parentEntity) {
                ...
                if (!store._ondataloaded) {
                    store.mon(store, 'load', function () {
                        me.fireEvent('ondataloaded');
                    });
                    store._ondataloaded = true;
                }
                var proxy = store.proxy;
...
Ext.define('SIE.view.QueryView', {
    ...
    resetCondition: function () {
        ...
        for (var i = 0; i < items.length; i++) {
            var _def = items[i]._default;
            if (typeof (_def) !== "undefined") {
                entity.set(items[i].name, _def);
            }
        }
    ...
    _executeQuery: function (opts) {
        try {
            ...
            opts = Ext.merge(opts || {}, { criteria: e });
...
Ext.define('SIE.autoUI.ViewFactory', {
     ...
     */
    _processConditionFormCfg: function (formCfg) {
        if (formCfg) {
            if (formCfg.items) {
    ...
    createConditionView: function (block) {
        ...
        this._processConditionFormCfg(formCfg);
        if (!formCfg.tbar) {
            formCfg.tbar = [{ command: 'SIE.cmd.ExecuteQuery' }, { command: 'SIE.cmd.ClearCondition' }, { command: 'SIE.cmd.ResetCondition' }];
        }
...
(function () {
    ...
    Ext.override(Ext.grid.ColumnModel, {
        ...
        //批量添加列
        addColumns: function (columnList, colIndex) {
            var grid = this.headerCt.grid;
            var gridColumns = grid.config.columns;

            columnList.forEach(function (c) { c.isDynamic = true; });//标记动态列

            if (typeof colIndex == 'number') {
                Ext.Array.insert(gridColumns, colIndex, columnList);
            } else {
                Ext.Array.insert(gridColumns, gridColumns.length - 1, columnList);
            }
        ...
        //批量删除列，删除所有列后再渲染。可支持不连续列
        removeColumns: function (colIndexList) {
            if (Ext.isEmpty(colIndexList))
                return;
            var grid = this.headerCt.grid;
            var gridColumns = grid.config.columns;
            for (var i = colIndexList.length - 1; i >= 0; i--) {
                //倒序删除
                var colIndex = colIndexList[i];
                var header = null;
                if (typeof colIndex == 'number') {
                    header = this.getHeaderAtIndex(colIndex);
                }
    ...
    Ext.override(Ext.grid.GridPanel, {
        ...
        //批量添加列
        addColumns: function (fields, columnList, colIndex) {
            this.store.addField(fields);
            this.columnManager.addColumns(columnList, colIndex);
        },
        ...
        //批量删除列-多列删除时，性能会比逐列删除高
        removeColumns: function (colIndexList) {
            if (Ext.isEmpty(colIndexList))
                return;
            var me = this;
            var dataIndexList = [];
            var fullColumnIndexList = [];
            colIndexList.forEach(function (colIndex) {
                var header = null;
                if (typeof colIndex != 'number') {
                    header = me.columnManager.getHeaderByDataIndex(colIndex);
                }
                else {
                    header = me.columnManager.getHeaderAtIndex(colIndex);
                }
                if (header) {
                    dataIndexList.push(header.dataIndex);
                    fullColumnIndexList.push(header.fullColumnIndex);
                }
            });
            if (dataIndexList.length > 0 && fullColumnIndexList.length > 0) {
                //倒序删除Field
                for (var i = dataIndexList.length - 1; i >= 0; i--) {
                    this.store.removeField(dataIndexList[i]);
                }
                //删除列
                this.columnManager.removeColumns(fullColumnIndexList);
            }
        },
        //实现列表悬浮框（gc）
...
Ext.define('SIE.ExportExcelHelper', {
    ...
    * @param mask 遮罩
    ...
    exportXls: function (cfg, datas, title, mask) {
        mask = mask || this.showMask(Ext.getBody().component, '下载中...'.t());
        var add_cols = function (col, cfg) {
            var a = col.addColumn({ text: cfg.header, width: cfg.width || 140 });
            if (cfg.columns) {
        ...
        var task = new Ext.util.DelayedTask(function () {
            ...
            var add_val = function (row, item, cfg) {
                if (cfg.columns)
                    cfg.columns.forEach(function (col) {
                        add_val(row, item, col);
...
Ext.define('portal_base', {
    ...
    getHeaderHeight: function () {
        return Ext.getCmp('portal-tabBar').height;
    },
    ...
    initLayout: function () {
        ...
        var _tabBar = {
            ...
            renderTpl: '<tpl if="hasTabGuard">{% this.renderTabGuard(out, values, \'before\'); %}</tpl>' +
                '<div class="ptb-logo"></div>' +
                '<div class="ptb-bg"></div>' +
                '<ul id="north-west-panel" class="north-west-panel">' +
                '<li id="btnNav"></li>' +
                '<li id="portal-btn-menu"></li>' +
                '</ul>' +
                '<ul id="north-east-panel" class="north-east-panel" style="right:-210px">' +
                '<li id="sysMenu">' +
                '</li>' +
                '<li id="btnOrg">' +
                '</li>' +
                '<li><span class="user-welcome-info"><label name="LoginUser"></label></span></li>' +
                '</ul>' +
                ...
                '<div id="{id}-strip" data-ref="strip" role="presentation" style="width:150%" class="{stripCls} {stripCls}-{ui}{childElCls}"></div>',
        };
        ...
        var tabPanel = {
            ...
            listeners: {
                afterlayout: {
                    fn: function (_this, eOpts) {
                        var width = _this.getWidth();
                        var bar = Ext.getCmp('portal-tabBar');
                        bar.setMaxWidth(width - 216);
                        bar.setWidth(width);
                    }
                }
            },
            plugins: {
                ...
                tabclosemenu: {
                    closeTabText: "关闭当前".t(),
                    closeAllTabsText: "关闭所有".t(),
                    closeOthersTabsText: "关闭其它".t()
                }
            },
            items: []
        };
    ...
    initNavMenu: function () {
        ...
        Ext.create('Ext.Button', {
            iconCls: 'iconfont icon-FunctionMenu navMenuIcon',
            text: '功能地图'.t(),
            renderTo: Ext.getElementById('btnNav'),
            border: false,
            arrowVisible: false,
            style: {
                padding: '9px 0px 9px 0px;'
            },
            handler: me.openNavTab
        });
    },
    ...
    _sysMenuArr: [
        {
            ...
            text: '关闭所有标签'.t(),
            handler: function () {
                portal.getTabPanel().items.each(function (item) {
                    if (item.closable) {
                        portal.getTabPanel().remove(item);
                    }
                });
            }
        },
        {
            ...
            text: '关闭当前标签'.t(),
            handler: function () {
                var item = portal.getTabPanel().getActiveTab();
        ...
        },
        {
            ...
            text: '关闭其它标签'.t(),
            handler: function () {
                var curItem = portal.getTabPanel().getActiveTab();
                portal.getTabPanel().items.each(function (item) {
                    if (item.closable && item !== curItem) {
                        portal.getTabPanel().remove(item);
                    }
                });
            }
     ...
     * 添加系统菜单(右上角)
     * @class portal_base
     * @param {json} menuInfo 菜单配置信息
     * @example portal.addSysMenu(
     * {
     *  index:1,
     *  text: '菜单1',
     *  renderTo: Ext.getBody(),
     ...
     * })
     */
    addSysMenu: function (menuInfo) {
        this._sysMenuArr.push(menuInfo);
    },

    /**
     * 打开菜单
     * @param {*} menuItem -菜单item
     */
    openMenu: function (menuItem) {
        switch (menuItem.Target) {
            case 0:
                CRT.Workbench.addPage({
                    entityType: menuItem.Url ? menuItem.id : menuItem.ModuleKey,
                    module: menuItem.keyLabel,
                    title: menuItem.title || menuItem.text,
                    url: menuItem.Url,
                    notload: menuItem.Url ? true : false//如果是http直接请求，就不load页签(解决页面一直加载问题)
                });
                break;
            case 2:
            ...
            case 3:
                if (!menuItem.Url) {
                    SIE.Msg.showError('该菜单项没有配置Url或打开方式配置不正确'.t());
                    return;
                }
                window.open(this.urlConvert(menuItem.Url), '');
                break;
            default:
    ...
    loadThemes: function () {
        var me = this;
        Ext.create('Ext.Button', {
            renderTo: Ext.getElementById('btnSkin'),
            iconCls: 'iconfont icon-Skin',
            arrowVisible: false,
            border: false,
            menu: {
                defaults: {
                ...
                items: GlobalConfig.themeConfig
            }
        });
    },
     ...
     * 加载系统菜单
     */
    loadSysMenu: function () {
        var me = this;
        me._sysMenuArr.sort(function (a, b) { return a.index - b.index; });
        Ext.create('Ext.Button', {
            renderTo: Ext.getElementById('sysMenu'),
            iconCls: 'iconfont icon-Menu',
            arrowVisible: false,
            border: false,
            menu: {
                id: 'portal-sysMenu',
                items: me._sysMenuArr
            }
        });
    },

    showAboutInfo: function () {
        var aboutPanel = Ext.create('Ext.window.Window', {
            title: '关于'.t(),
            width: 580,
            height: 400,
            layout: 'fit',
            items: {
                ...
                items: [{
                    region: 'north',
                    height: 140,
                    html: Ext.get('aboutHeader').dom.innerHTML,
                    border: false
                }, {
                    region: 'south',
                    height: 50,
                    html: Ext.get('aboutFooter').dom.innerHTML,
                    margin: '2 0 0 0',
                    border: false
                }, {
                    title: false,
                    region: 'center',
                    html: Ext.get('aboutContent').dom.innerHTML,
                    border: false
                }],
            },
            modal: true,
    ...
    setWorkspace: function () {
        var me = this;
        SIE.App.setWorkspaceProvider({
            _tab: function () {
                return me.getTabPanel();
            },
            getModules: function () {
                var tab = this._tab();
                var res = [];
                SIE.each(tab.items, function (tabItem) {
                    res.push(tabItem.id);
                });
                return res;
            },
            ...
            addModule: function (module, moduleControl, tab) {
                var tabPanel = this._tab();
                var exists = Ext.getCmp(tab.id);
                if (exists) {
                    tabPanel.setActiveTab(exists);
                } else {
    ...
    /**
     * 添加页签
     * @param {any} item -页签item
     * @param  item.id/item.mid 页签ID
     * @param  item.url 以URL的形式打开
     * @param  item.method 以URL打开页签访问的方式 GET/POST
     * @param  item.params 以URL打开页签访问的方式POST的参数数据 如 {username:'test',password:'888'}
     * @param  item.control 以控件的方式打开页签 页签内容
     * @param  item.keyLabel 以控件的方式打开页签 模块KEY
     */
    addTab: function (item) {
        var tabPanel = this.getTabPanel();
        var rawId = item.mid || item.id;
        var isLoadedCloseTab = item.isLoadedCloseTab || false;
        var tabId = ('tab_' + rawId).replace(/[.|,]/g, '');
        var tab = Ext.getCmp(tabId);
        ...
        if (!tab) {
            ...
            if (url) {
                url = this.urlConvert(url);
                var iframeId = tabId + '_iframeEl';
                //tab.html = Ext.String.format('<iframe src="{0}" id="{1}"  data-ref="iframeEl" width="100%" height="99%" scrolling="auto" frameborder="0"  ></iframe>', url, iframeId);
                tab.html = Ext.String.format('<iframe  id="{0}" name="{0}" data-ref="iframeEl" width="100%" height="99%" scrolling="auto" frameborder="0"  ></iframe>', iframeId);
                tab.view = item.view;
                tab = tabPanel.add(tab);
                tabPanel.setActiveTab(tab);
                var tempForm = document.createElement("form");
                //制定发送请求的方式为post  
                tempForm.method = item.method || "GET";
                //此为window.open的url，通过表单的action来实现  
                tempForm.action = url;
                //利用表单的target属性来绑定window.open的一些参数（如设置窗体属性的参数等）  
    ...
    initTheme: function () {
        var theme = LocalStorageHelper.getJsonData('theme');
        if (theme) {
            this.setTheme(theme);
        } else {
            this.updateLayout();
    ...
    setTheme: function (item) {
        var eleCurTheme = document.getElementById('theme');
        if (eleCurTheme.href.indexOf(item.extThemeUrl) > -1) return;
        portalSpinner.show(document.body);
        this.setExtTheme(item.extThemeUrl);
        this.setPortalTheme(item.siteThemeUrl);
        var theme = {
            id: item.id,
            text: item.text,
            extThemeUrl: item.extThemeUrl,
            siteThemeUrl: item.siteThemeUrl
        };
        LocalStorageHelper.setJsonData('theme', theme);
    },
     ...
     * @param {any} url -url
     */
    setExtTheme: function (url) {
        var me = this;
        var eleTheme = CssHelper.swapStyleSheet("theme", url);
        var edgeLoadTime = 1; //防止Edge浏览器执行两次
        eleTheme.onload = function (e) {
            if (Ext.isEdge) {
                if (edgeLoadTime < 2) {
                    edgeLoadTime++;
                    setTimeout(function () {
     ...
     * @param {any} url -url
     */
    setPortalTheme: function (url) {
        var me = this;
        var eleTheme = CssHelper.swapStyleSheet("portaltheme", url);
        eleTheme.onload = function (e) {
            me.onThemeLoad();
        }
    },
    ...
    initSessionState: function () {
        ...
        CRT.Context.GlobalContext.setContext(this.userKey, curUser);
    },

    /**
     * 初始化Ext语言包
     */
    initExtLocale: function () {
        var curUser = CRT.Context.GlobalContext.getContext(this.userKey);
        var jsPath;
        if (curUser.Culture) {
            SIE.App.CurCulture = curUser.Culture;
            jsPath = '/ExtJs/locale/locale-' + curUser.Culture + '.js';
            FileHelper.swapJsFile('extlocale', jsPath);
        } else {
     ...
     *  初始化SIE资源
     * */
    initSieResource: function () {
        if (SIE.App.CurCulture) {
            this.getResource(SIE.App.CurCulture);
        }
    },
    getResource: function (cultureCode, resources) {
        var resourceKey = 'sie_culture_resource_' + cultureCode;
        var localResource = LocalStorageHelper.getJsonData(resourceKey);
        var version = -1;
        if (localResource) {
            version = localResource.Version;
        }
        Ajax('/api/LocalizationApi/GetCultureResource',
            {
                method: 'get',
                isAsync: 'true',
                timeout: 300000,
                ...
                onSuccess: function (res) {
                    ...
                    if (jsonRes.Success) {
                        var responseResource = null;
                        if (jsonRes.Result.Resources) {
                            responseResource = jsonRes.Result;
                            if (responseResource) {
                                try {
                                    if (localResource === '') {
                                        localResource = responseResource;
                                    } else {
                                        // step3 增量,有则更新，没有则添加
                                        // 要考虑浏览器兼容性写法 参考:https://www.runoob.com/jsref/jsref-obj-array.html
                                        if (SIE.isArray(responseResource.Resources)) {
                                            responseResource.Resources.forEach(function (i) {
                                                var exist = localResource.Resources.some(function (l) {
                                                    var isSome = i.Key === l.Key;
                                                    if (isSome) {
                                                        l.Value = i.Value;
                                                    }
                                                    return isSome;
                                                ...
                                                if (!exist) {
                                                    localResource.Resources.push(i);
                                                }
                                            });
                                        }
                                    }
                                    localResource.Version = responseResource.Version;
                                    LocalStorageHelper.setJsonData(resourceKey, localResource);
                                } catch (e) {
                                    console.error(e.message);
                                }
                            }
                            localization.loadStoregeResource();
                        }
                    }
                },
                onError: function (o) {
    ...
    refreshUserInfo: function () {
        var elems = Ext.query('label[name=LoginUser]');
        Ext.Array.each(elems, function (el) {
            el.innerText = '您好,'.t() + SIE.App.getUserDisplayName();
        });
    },
     ...
     * 添加左侧菜单项
     * @param {string} id -id
     * @param {any} config -button配置
     * @return {any} -btn
     */
    addWestMenu: function (id, config) {
        var ul = document.getElementById('north-west-panel');
        var li = document.createElement('li');
        li.id = id;
        ul.appendChild(li);
        Ext.apply(config, {
            renderTo: Ext.getElementById(id)
        });
     ...
     * @return {any} -btn
     */
    addEastMenu: function (id, config) {
        var ul = document.getElementById('north-east-panel');
        var li = document.createElement('li');
        li.id = id;
        ul.appendChild(li);
        Ext.apply(config, {
    ...
    toggleFullscreen: function () {
        var cmpNorth = Ext.getCmp('portal-north');
        var cmpSouth = Ext.getCmp('portal-south');
        var isNorthHidden = cmpNorth.isHidden();
        var isSouthHidden = cmpSouth.isHidden();
        if (isNorthHidden) {
            cmpNorth.show();
        } else {
            cmpNorth.hide();
    ...
    //从URL读取参数并打开相关页面
    _loadPageFromUrl: function () {
        CRT.Workbench.loadPageFromUrl();
    },
    ...
    loginSuccessed: function () {
        ...
        //从URL读取参数并打开相关页面
        this._loadPageFromUrl();
        this.getTabPanel().loaded = true;
    },
    ...
    init: function () {
        ...
        document.getElementById('south-panel').style.display = 'block';
    }
