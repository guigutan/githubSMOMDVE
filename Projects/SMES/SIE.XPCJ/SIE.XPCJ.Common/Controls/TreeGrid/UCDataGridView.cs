    ...
    public partial class UCDataGridView : UserControl
    {
        ...
        private void LoadColumns()
        {
            try
            {
                ...
                if (m_columns != null && m_columns.Count() > 0)
                {
                    var absSum = m_columns.Where(p => p.WidthType == SizeType.Absolute).Sum(p => p.Width);
                    int _width = 0;
                    m_columns.ForEach(p =>
                    {
                        ...
                        {
                            _width += (int)((p.Width / 100f) * (this.Width- absSum));
                        }
                    });
                    if (m_isShowCheckBox)
                        _width += 30;
                    ...
                    for (int i = 0; i < intColumnsCount; i++)
                    {
                        ...
                        if (i == 0 && m_isShowCheckBox)
                        {
                            ...
                            box.CheckedChanged += (a, b) =>
                            {
                                ...
                                ARowCheckedChanged?.Invoke(a, null);
                            };
                            c = box;
                            HeaderCheckBox = box;
                        }
                        else
                        {
                            ...
                            lbl.TextAlign = item.HeadTextAlign;// ContentAlignment.MiddleCenter;
                            lbl.AutoSize = false;
        ...
        public event DataGridViewEventHandler ARowCheckedChanged;

        public void TriggerARowCheckedChanged(object sender, int rowIndex)
        {
            DataGridViewEventArgs e = new DataGridViewEventArgs
            {
                RowIndex = rowIndex
            };
            ARowCheckedChanged?.Invoke(sender, e);
        }
        ...
        public void ReloadSource()
        {
            ...
            try
            {
                ...
                if (m_dataSource != null)
                {
                    ...
                    this.panRow.Controls.Clear(); //zzx 全部清空重新创建

                    foreach (Control item in this.panRow.Controls)
                    {
                        if (item is UCDataGridViewTreeRow)
                        {
                            (item as UCDataGridViewTreeRow).RowLevel = 0;
                            (item as UCDataGridViewTreeRow).IsShowCheckBox = true;
                            (item as UCDataGridViewTreeRow).ChildrenRows.Clear();
                        }
                    ...
                    if (intIndex < intSourceCount)
                    {
                        for (int i = intIndex; i < intSourceCount; i++)
                        {
                            ...
                            row.IsShowCheckBox = m_isShowCheckBox;
                            row.IsShowChildRowCheckBox = IsShowChildRowCheckBox;
                            row.ReloadCells();
                            row.BindingCellData();
                            ...
                            row.CellClick += (a, b) => {
                                ...
                                ARowCheckedChanged?.Invoke(rowControl, b); 
                            };
                            row.CheckBoxChangeEvent += (a, b) => { 
                                SetSelectRow(rowControl, b); 
                                ARowCheckedChanged?.Invoke(rowControl, b); 
                            };
