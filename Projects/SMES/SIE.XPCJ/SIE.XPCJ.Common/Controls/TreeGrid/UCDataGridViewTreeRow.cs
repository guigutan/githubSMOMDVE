    ...
    public partial class UCDataGridViewTreeRow : UserControl, IDataGridViewRow
    {
        ...
        public int RowLevel
        {
            ...
            set
            {
                ...
                int paddingLeft = this.panLeft.Width / 2 * value + value > 0 ? 30 : 0;
                this.Padding = new Padding(paddingLeft, this.Padding.Top, this.panLeft.Right, this.Padding.Bottom);
        ...
        public bool IsShowChildRowCheckBox { get; set; }
        ...
        public int RowHeight
        {
            ...
            {
                m_rowHeight = value;
                this.Height = m_actRowHeightCount * value;
            }
        }
            ...
            for (int i = 0; i < Columns.Count; i++)
            {
                ...
                if (cs != null && cs.Length > 0)
                {
                    ...
                    if (pro != null)
                    {
                        ...
                        if (com.Format != null)
                        {
                            cs[0].Text = i > 0 ? this.preSpace + com.Format(value) : com.Format(value);
                        }
                        else
                        {
                            cs[0].Text = i > 0 ? this.preSpace + value?.ToString() : value?.ToString();
                        }

                        SizeF textSize = MeasureString(cs[0].Text, cs[0].Font);
                        int a = (int)textSize.Width / (int)cs[0].Width;
                        int b = (int)textSize.Width % (int)cs[0].Width;
                        m_actRowHeightCount = Math.Max(m_actRowHeightCount, b > 0 ? a + 1 : a);

                    }
                }
            }

            this.Height = m_actRowHeightCount * m_rowHeight;
        ...
        void Item_MouseDown(object sender, MouseEventArgs e)
        {
            if (CellClick != null)
            {
                ...
                {
                    CellControl = this,
                    CellIndex = Convert.ToInt32((sender as Control).Tag)
                });
            }
        }
                ...
                if (Columns != null && intColumnsCount > 0)
                {
                    ...
                    for (int i = 0; i < intColumnsCount; i++)
                    {
                        ...
                        if (i == 0 && IsShowCheckBox)
                        {
                            ...
                            box.CheckedChanged += box_CheckedChangeEvent;
                            c = box;
                        }
                        else
                        {
        ...
        void box_CheckedChangeEvent(object sender, EventArgs e)
        {
            ...
            (this.Parent.Parent as UCDataGridView)?.TriggerARowCheckedChanged(sender, this.RowIndex);
        }
        ...
        private void panLeft_MouseDown(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (!IsOpened.Value)
                {
                    ...
                    IsOpened = !IsOpened;
                    if (childrenRows.Count > 0 && IsShowChildRowCheckBox)
                    {
                    ...
                    else
                    {
                        ...
                        if (proChildrens != null)
                        {
                            ...
                            for (int i = intSourceCount - 1; i >= 0; i--)
                            {
                                ...
                                row.Columns = Columns;
                                row.IsShowCheckBox = IsShowCheckBox && IsShowChildRowCheckBox;
                                row.ReloadCells();
                                row.BindingCellData();
                ...
                else
                {
                    ...
                    this.Height = this.m_actRowHeightCount * m_rowHeight;
                }
            }
            finally
            {
