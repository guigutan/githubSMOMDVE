using System.ComponentModel;
using System.Reflection;

namespace SIE.CrossPlatform.Collect.Common.Extensions
{
    public static class SystemExtension
    {
        /// <summary>
        /// 获取<see cref="SIE.ObjectModel.LabelAttribute"/>标记的文本
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public static string ToLabel(this Enum value)
        {
            if (value == null)
                return null;
            return EnumViewModel.EnumToLabel(value).L10N();
        }
    }
    /// <summary>
    /// 枚举值的逻辑视图
    /// </summary>
    public class EnumViewModel
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="value">枚举值</param>
        public EnumViewModel(Enum value)
        {
            this.EnumValue = value;
            this.Category = GetEnumCategory(value);
            this.Label = EnumToLabel(value);
            this.TranslatedLabel = this.Label;
        }

        private EnumViewModel(Enum value, string label)
        {
            this.EnumValue = value;
            this.Label = label;
            this.TranslatedLabel = this.Label;
        }

        /// <summary>
        /// 枚举值
        /// </summary>
        public Enum EnumValue { get; private set; }

        /// <summary>
        /// 枚举值的显示值
        /// </summary>
        public string Label { get; private set; }

        /// <summary>
        /// 翻译后的枚举值的显示值
        /// </summary>
        public string TranslatedLabel { get; private set; }

        /// <summary>
        /// 获取枚举值的分类
        /// </summary>
        /// <value>
        /// 分类值
        /// </value>
        public string Category { get; private set; }

        /// <summary>
        /// EnumPropertyEditor 和其它地方中使用了这个方法来对比实体。
        /// </summary>
        /// <param name="obj"></param>
        /// <returns></returns>
        public override bool Equals(object obj)
        {
            if (obj is EnumViewModel)
                return EnumValue.Equals((obj as EnumViewModel).EnumValue);
            else
                return false;
        }

        public override int GetHashCode()
        {
            return EnumValue.GetHashCode();
        }

        public override string ToString()
        {
            return Label;
        }

        /// <summary>
        /// 把 Label 解析为目标枚举类型中对应的枚举值。
        /// </summary>
        /// <param name="str"></param>
        /// <param name="enumType">枚举类型（不接受可空类型）</param>
        /// <returns></returns>
        public static object LabelToEnum(string str, Type enumType)
        {
            FieldInfo[] fieldInfos = enumType.GetFields();

            foreach (var item in fieldInfos)
            {
                var attr = item.GetSingleAttribute<LabelAttribute>();
                if (attr != null && attr.Label == str)
                {
                    return Enum.Parse(enumType, item.Name, true);
                }
            }

            //如果在 Label 中没有找到，那这个 label 其实就是枚举本身的名称。
            foreach (var item in fieldInfos)
            {
                if (item.Name == str)
                {
                    return Enum.Parse(enumType, item.Name, true);
                }
            }

            return null;
        }

        /// <summary>
        /// 返回一个枚举值对应的 Label。
        /// 如果该枚举值没有标记 Label，则返回 String.Empty。
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public static string EnumToLabel(Enum value)
        {
            if (value != null)
            {
                Type type = value.GetType();
                FieldInfo fieldInfo = type.GetField(value.ToString());
                if (null != fieldInfo)
                {
                    var attri = fieldInfo.GetSingleAttribute<LabelAttribute>();
                    if (attri != null)
                    {
                        return attri.Label;
                    }
                }
            }
            return null;
        }
        /// <summary>
        /// 获取枚举的分类
        /// </summary>
        /// <param name="value">检举值.</param>
        /// <returns></returns>
        public static string GetEnumCategory(Enum value)
        {
            if (value != null)
            {
                Type type = value.GetType();
                FieldInfo fieldInfo = type.GetField(value.ToString());
                if (null != fieldInfo)
                {
                    var attri = fieldInfo.GetSingleAttribute<CategoryAttribute>();
                    if (attri != null)
                    {
                        return attri.Category;
                    }
                }
            }
            return null;
        }
    }
    /// <summary>
    /// 标签属性
    /// </summary>
    public class LabelAttribute : Attribute
    {
        public string Label { get; private set; }

        public LabelAttribute(string label)
        {
            Label = label;
        }
    }
}
