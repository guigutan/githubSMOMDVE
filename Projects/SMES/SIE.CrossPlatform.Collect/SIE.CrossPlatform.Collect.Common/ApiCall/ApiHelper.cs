...
using SIE.CrossPlatform.Collect.Common.Log;
...
namespace SIE.CrossPlatform.Collect.Common.ApiCall
{
    public class ApiHelper
    {
        ...
        public static async Task Login(delegateApiCallback<LoginInfo> onApiCallback)
        {
            delegateApiCallback<LoginInfo> callback = new delegateApiCallback<LoginInfo>((result, apiType, method, postData) =>
            {
                if (result.Success)
                {
                    ...
                    LoginInfo.Instance.Context = "{'Ticket':'" + result.Context.Ticket + "','InvOrgId':'" + result.Context.InvOrgId
            ...
            await PostAsync<LoginInfo>("AuthenticationController", "Login", callback, LoginInfo.Instance.UserCode, LoginInfo.Instance.Password);
        }
        ...
        private static ApiResult<T> ReLogin<T>(string preUrl, string prePostData, ApiResult<T> preResult)
        {
            ...
            if (loginResult.Success)
            {
                ...
                LoginInfo.Instance.Context = "{'Ticket':'" + loginResult.Context.Ticket + "','InvOrgId':'" + loginResult.Context.InvOrgId
                   + "','Language':'" + (Global.Language == null ? "zh-CN" : Global.Language.Code) + "'}";
        ...
        /// <param name="onApiCallback"></param>
        /// <param name="parameters"></param>
        public static async Task PostAsync<T>(string apiType, string method, delegateApiCallback<T> onApiCallback, params object[] parameters)
        {
            StringBuilder sbParameters = new StringBuilder();
            if (parameters != null && parameters.Length > 0)
            {
            ...
            await _PostAsync(url, apiType, method, postData, onApiCallback, AppSettings.Instance.ApiTimeOut, contentType: "application/json", log: AppSettings.Instance.ApiLog);
        }
        ...
        private static async Task _PostAsync<T>(string url, string apiType, string method, string data, delegateApiCallback<T> onApiCallback, int timeout = 30000, Dictionary<string, string> headerDic = null, string contentType = "application/x-www-form-urlencoded", bool log = true)
        {
            if (log)
            {
            ...
            try
            {
                using (HttpClient httpClient = new HttpClient())
                {
                    ...
                    {
                        throw new  ValidationException("erro:HttpResponseCode:" + (int)response.StatusCode);
                    }
                    ...
                    {
                        LoginInfo.Instance.Context = "{'Ticket':'" + apiResult.Context.Ticket + "','InvOrgId':'" + apiResult.Context.InvOrgId
                            + "','Language':'" + (Global.Language == null ? "zh-CN" : Global.Language.Code) + "'}";
                    }
        ...
        public static void GetAsync(string url, delegateApiGetCallback onApiGetCallback)
        {
            if (!url.StartsWith("http::") && !url.StartsWith("https::"))
                url = http + url;
