...
namespace SIE.CrossPlatform.Collect.Common.Snowflakes
{
    /// <summary>
    /// 雪花扩展方法
    /// </summary>
    public static class SnowflakeExtension
    {
        ...
        static class SnowflakeStatic
        {
            ...
            /// <summary>
            /// 表示时间戳开始纪元。现为2021.06.01（可使用到2038.11.1前不会js精度丢失）
            /// </summary>
            public const long TW_EPOCH = 1622476800000L;

            /// <summary>
            /// 表示工作机器编号的位数。
            /// </summary>
            public const int WORKER_ID_BITS = 4;
            /// <summary>
            /// 表示数据中心编号的位数。
            /// </summary>
            public const int DATACENTER_ID_BITS = 4;
            /// <summary>
            /// 表示序列的字节数。
            /// 支持单机每秒生成63000个ID
            /// </summary>
        ...
        public class Snowflake
        {
            ...
            public static long FromBase(string value)
            {
                ...
                var radix = SnowflakeStatic.CHARACTERS.Length;
                long result = GetCharacterIndex(value[value.Length - 1]);

                for (int i = 1; i < value.Length; i++)
                {
                    result += (long)Math.Pow(radix, i) * GetCharacterIndex(value[value.Length - (i + 1)]);
                }
            ...
            public static Snowflake Create(IPAddress address, long sequence = 0)
            {
                try
                {
                    ...
                    var workerId = value % 16;
                    var datacenterId = (value - workerId) / 64;
                    return new Snowflake(workerId, datacenterId, sequence);
                }
                catch (Exception ex)
                {
                    Console.WriteLine("SnowflakeCreate Exception:" + ex.Message);
                    return null;
                }
            }
        }
