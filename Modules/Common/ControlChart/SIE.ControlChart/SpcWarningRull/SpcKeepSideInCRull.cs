...
namespace SIE.ControlChart.SpcWarningRull
{
    ...
    /// </summary>
    [Serializable]
    public class SpcKeepSideInCRull : SpcRull
    {
        /// <summary>
        /// 均值图上CB分区值
        /// </summary>
        ...
        /// </summary>
        /// <param name="n"></param>
        public SpcKeepSideInCRull(int? n = null)
        {
            RullDescription = "连续{0}点，距离中心线（任一侧）1个标准差以内".L10nFormat(n);
            if (n.HasValue)
                N = n.Value;
        }
        /// <summary>
            ...
            for (var i = 0; i < Datas.Count; i++)
            {
                ...
                else
                {
                    if (count >= N)
                    {
                        ...
                        KeepSideInCPointsViolationRull(violateRullEvents, violateRullDatas);
                    }
        ...
        /// <summary>
        /// 连续K点，距离中心线（任一侧）1个标准差以内（K为违反SPC判异规则的连续数据点数）
        /// </summary>
        /// <param name="violateRullEvents">违反SPC判异规则事件</param>
        /// <param name="violateRullDatas">违反SPC判异规则的数据点集合</param>
        private void KeepSideInCPointsViolationRull(List<ViolateSpcRullEvent> violateRullEvents, List<Tuple<double, DateTime?>> violateRullDatas)
        {
