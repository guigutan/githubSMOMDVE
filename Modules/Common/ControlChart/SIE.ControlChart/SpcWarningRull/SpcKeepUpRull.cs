...
namespace SIE.ControlChart.SpcWarningRull
{
    ...
    /// </summary>
    [Serializable]
    public class SpcKeepUpRull : SpcRull
    {
        /// <summary>
        /// 连续n个点保持递增构造函数
        /// </summary>
        /// <param name="n"></param>
        public SpcKeepUpRull(int? n = null)
        {
            RullDescription = "连续{0}点，连续递增".L10nFormat(n);
            if (n.HasValue)
                N = n.Value;
        }
        ...
        public override List<ViolateSpcRullEvent> GetDrawInfo()
        {
            ...
            for (var i = 1; i < Datas.Count; i++)
            {
                ...
                else
                {
                    if (count >= N - 1)
                    {
                        ...
                        KeepUpPointsViolationRull(Datas, violateRullEvents, count, i);
                    }
                    count = 0;
                }
            ...
            if (count >= N - 1)
            {
                ...
                KeepUpPointsViolationRull(Datas, violateRullEvents, count, Datas.Count);
            }
            return violateRullEvents;
        }
        /// <summary>
        ...
        /// <param name="i"></param>
        private void KeepUpPointsViolationRull(List<DataPoint> datas, List<ViolateSpcRullEvent> violateRullEvents, int count, int i)
        {
            if (count >= N - 1)
            {
                if (count > N - 1)
