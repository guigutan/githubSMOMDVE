...
namespace SIE.ControlChart.ChartDatas.SpcUCharts
{
    ...
    /// </summary>
    [Serializable]
    public class UData : CountingChartData
    {
        /// <summary>
        /// 子组容量
        /// </summary>
        ...
        public override void Calculate()
        {
            ...
            Datas.ForEach(d =>
            {
                ...
                if (hasCustomControlLine)
                {
                    ...
                    {
                        cl.Ucl = Math.Max(0, DefineControlLine.Ucl);
        ...
        {
            return avg + 3 * Math.Sqrt(avg / sampleQty.ToDouble());
        }

        private double GetLcl(double avg, int sampleQty)
        {
            return Math.Max(0, avg - 3 * Math.Sqrt(avg / sampleQty.ToDouble()));
        }
    }
}
