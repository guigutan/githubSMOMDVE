...
namespace SIE.Wpf.WorkBenchCommon.Editors
{
    ...
    /// 指标名称编辑器
    /// </summary>
    public class QuotaNameEditor : BaseEditor<QuotaNameEditorConfig>
    {
        /// <summary>
        /// 编辑器名称
        /// </summary>
        public static readonly string EditorName = "QuotaNameEditor";
        ...
        {
            string upperLevelCategory = string.Empty;
            if (Config.UpperLevelProperty != null)
            {
                string upperProperty = source.GetProperty(Config.UpperLevelProperty).ToString();
                upperLevelCategory = upperProperty.IsNullOrEmpty() ? " " : upperProperty;
            }

            List<string> dataSource = GetQuotaNameList(upperLevelCategory);
        ...
        /// 根据上一级指标分类获取下一级指标名称
        /// </summary>
        /// <param name="upperLevelCategory">上一级指标分类</param>
        /// <returns>指标名称</returns>
        public List<string> GetQuotaNameList(string upperLevelCategory)
        {
            List<string> nameList = new List<string>();
            if (!string.IsNullOrEmpty(upperLevelCategory))
            {
                var types = CRT.GetAllModules().SelectMany(p => p.Assembly.GetTypes());
                foreach (var type in types)
                {
                    var definition = type.GetCustomAttribute<QuotaAttribute>();
                    if (definition != null && upperLevelCategory == definition.Code && !nameList.Contains(definition.Name))
                    {
    ...
    /// 指标名称配置类
    /// </summary>
    public class QuotaNameEditorConfig : BaseEditorConfig
    {
        /// <summary>
        /// 是否每次下拉时重新加载数据
        /// </summary>
