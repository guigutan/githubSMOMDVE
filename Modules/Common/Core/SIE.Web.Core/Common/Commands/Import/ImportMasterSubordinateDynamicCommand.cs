...
namespace SIE.Web.Core.Common.Commands.Import
{
    ...
    /// 主从表(动态列)导入命令
    /// </summary>
    public abstract class ImportMasterSubordinateDynamicCommand : ImportCommandBase
    {
        /// <summary>
        /// 
        /// </summary>
        protected ImportMasterSubordinateDynamicCommand()
        {
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual MasterSubordinateDynamicImportData CreateMasterSubordinateDynamicImportData() {
            return new MasterSubordinateDynamicImportData();
        }
        ...
        {
            //1.创建主从结构信息
            MasterSubordinateDynamicImportData import = this.CreateMasterSubordinateDynamicImportData();
            ...
            //4.获取孙表的Sheet数据
            foreach (var grandsonSheetName in import.GrandsonSheetNameList)
            {
                DataTable table = GetDataTable(args.Data, grandsonSheetName);
                table.TableName = grandsonSheetName;
                import.GrandsonDataTables.Add(table);
            }

            //5.创建主从表导入的处理对象
            ImportMasterSubordinateDynamicDataHandle handle = new ImportMasterSubordinateDynamicDataHandle();
