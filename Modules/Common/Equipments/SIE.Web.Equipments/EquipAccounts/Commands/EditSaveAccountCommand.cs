...
namespace SIE.Web.Equipments.EquipAccounts.Commands
{
    ...
    /// 设备台账修改保存命令
    /// </summary>
    [JsCommand("SIE.Web.Equipments.EquipAccounts.Commands.EditSaveAccountCommand")]
    public class EditSaveAccountCommand : FormSaveCommand
    {
        /// <summary>
        /// 保存设备台账
        /// </summary>
        ...
        {
            var equip = entity as EquipAccount;
        ...
        protected void ValidateEquipmentAccount(EquipAccount equipAccount)
        {
            if (equipAccount == null)
            {
                throw new ValidationException("要保存设备台账实体为空".L10N());
            }
            if (string.IsNullOrEmpty(equipAccount.Name))
            {
            ...
            //}
            //if (!equipAccount.ManageDepartmentId.HasValue)
            //{
        ...
        /// <returns></returns>
        protected EquipAccountResume GenerateEquipAccountResume(string changed, AccountState state, string oldValue, string newValue)
        {
        ...
        /// <returns></returns>
        protected EntityList<EquipAccountResume> GenerateEquipAccountResumes(EquipAccount newEntity)
        {
            var oldEntity = RF.GetById<EquipAccount>(newEntity.Id);
