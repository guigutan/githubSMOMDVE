SIE.defineCommand('SIE.Web.Inventory.Task.Commands.PrintTaskManagementCommand', {
    ...
    execute: function (view, source) {
        ...
        if (view.getSelection().any(function (f) { return f.data.OrderType != orderType })) {
            ...
            SIE.invokeDataQuery({
                async: false,
                type: "SIE.Web.Inventory.Task.DataQueryer.TaskManagementDataQueryer",
                method: 'GetTaskManagementNumberRule',
                token: view.token,
                params: [orderType],
    ...
    showWin: function (view, entityData, billIds) {
        SIE.AutoUI.getMeta({
            model: 'SIE.Inventory.Task.ViewModels.TaskManagementPrintViewModel',
            ignoreCommands: true,
            isDetail: true,
            ignoreQuery: false,
