SIE.defineCommand('SIE.Web.Resources.UserGroups.Commands.SelectInvOrgCommand', {
    extend: 'SIE.cmd.LookupCommandBase',
    userConfig: {
        dataParams: { specKeyPrototyName: 'InvId', targetClassName: 'SIE.Rbac.InvOrgs.InvOrg' }
    },
        ...
        if (selections && selections.items.length > 0) {
            ...
            SIE.each(selections.items, function (sel) {
                ...
                if (me._sourceViewSelectItems.indexOf(id) === -1) {
                    var item = { UserGroupId: me._sourceId, InvId: id };
                    items.push(item);
                }
            });
            if (items.length > 0) {
