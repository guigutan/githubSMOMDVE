SIE.defineCommand('SIE.Web.LES.StockPlans.Commands.ForceCompleteCommand', {
    meta: { text: "强制完成", group: "edit", iconCls: "icon-SettingFinish icon-blue" },
    extends: "SIE.Web.ShipPlan.Commands.ForceCompleteCommand",
    canExecute: function (view) {
        ...
        if (sel.any(function (p) {
            return p.getState() !== SIE.Web.ShipPlan.DeliveryState.Aduited.value &&
                p.getState() !== SIE.Web.ShipPlan.DeliveryState.Executing.value || p.getNoCreateQty() <= 0;
        })) {
            return false;
        }
        return true;
