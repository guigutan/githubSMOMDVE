...
namespace SIE.LES.PrepareItems
{
    ...
    /// 导入备料模式维护-推式
    /// </summary>
    [Services.Service(FallbackType = typeof(PrepareItemPushImportHandle), ServiceLifeStyle = Services.ServiceLifeStyle.Transient)]
    public class PrepareItemPushImportHandle : IDisposable, IBusinessImport
    {
        /// <summary>
        /// 导入列名
        /// </summary>
        ...
        {
            "生产资源编码","物料类型","物料编码","物料扩展属性","触发方式","提前小时","最短满足时间（小时）","需求计算方式","固定量","最小备料时间（小时)"
        };
        ...
        /// 生产资源
        /// </summary>
        private Dictionary<string, WipResource> dicWipResource { get; set; }
        ...
        public IBusinessImport CreaetColumnValid()
        {
            ...
            this.ColumnValidList.Add(ColumnNameList[0], new ValidColumn(ImportDataType._String, true, VaildWipResourceCode));// 生产资源编码
            ...
            this.ColumnValidList.Add(ColumnNameList[4], new ValidColumn(ImportDataType._Enum, true, true));//触发方式
            this.ColumnValidList.Add(ColumnNameList[5], new ValidColumn(ImportDataType._String, false, true));//提前小时
            this.ColumnValidList.Add(ColumnNameList[6], new ValidColumn(ImportDataType._String, false, true));//最短满足时间
            this.ColumnValidList.Add(ColumnNameList[7], new ValidColumn(ImportDataType._Enum, true, true));//需求计算方式
            this.ColumnValidList.Add(ColumnNameList[8], new ValidColumn(ImportDataType._String, false, true));//固定量
            this.ColumnValidList.Add(ColumnNameList[9], new ValidColumn(ImportDataType._String, false, true));//最小备料时间
            return this;
        }
        ...
        private bool VaildItemCode(object obj, out string messageTip, DataRow dr)
        {
            ...
            {
                var item = RT.Service.Resolve<ItemController>().GetAllEnableItems(null, itemCode, ConsumeMode.Push).FirstOrDefault();
        ...
        /// 验证生产资源编码
        /// </summary>
        /// <param name="obj">生产资源编码</param>
        /// <param name="messageTip">信息提示</param>
        /// <param name="dr">当前行数据</param>
        /// <returns>是否验证通过</returns>
        private bool VaildWipResourceCode(object obj, out string messageTip, DataRow dr)
        {
            bool isValid = true;
            if (dicWipResource == null)
            {
                dicWipResource = new Dictionary<string, WipResource>();
            }

            string wipResourceCode = obj.ToString();
            ...
            string uniquenessData = wipResourceCode + "-" + itemCategoryCode + "-" + itemCode + "-" + itemExtPropName + "-" + triggerType;

            if (!dicWipResource.ContainsKey(uniquenessData))
            {
                var wipResource = RT.Service.Resolve<WipResourceController>().GetScheResourceByResCode(wipResourceCode);
                if (wipResource == null)
                {
                    messageTip = "不存在于系统".L10N();
                    AppendErrorMsg(dr, ImportDataHandle.MessageColumnName, messageTip);
                ...
                dicWipResource.Add(uniquenessData, wipResource);
            }
            else
            {
        ...
        public void ProcessBusinessDataHandle(DataRow[] drs)
        {
            ...
            var mainDataList = from g in drs
                               select new
                               {
                                   WipResourceCode = g.Field<string>(ColIndex("生产资源编码")),
                                   ...
                                   AdvanceHours = g.Field<string>(ColIndex("提前小时")),
                                   SatisfactionTime = g.Field<string>(ColIndex("最短满足时间（小时）")),
                                   DemandType = g.Field<string>(ColIndex("需求计算方式")),
                                   FixedQuantity = g.Field<string>(ColIndex("固定量")),
                                   PreparationTime = g.Field<string>(ColIndex("最小备料时间（小时)")),
                                   DetailInfo = g
                               };
            ...
            foreach (var mainDataItem in mainDataList)
            {
                try
                {
                    var wipResource = mainDataItem.WipResourceCode + "-" + mainDataItem.ItemCategoryCode + "-" + mainDataItem.ItemCode + "-" + mainDataItem.ItemExtPropName + "-" + mainDataItem.TriggerType;
                    var prepareItemPush = new PrepareItemPush();
                    prepareItemPush.WipResourceId = dicWipResource[wipResource].Id;
                    if (!mainDataItem.ItemCategoryCode.IsNullOrEmpty())
                    {
                        prepareItemPush.ItemCategoryId = dicItemCategory?[mainDataItem.ItemCategoryCode].Id;
                    }
                    if (!mainDataItem.ItemCode.IsNullOrEmpty())
                    {
                        prepareItemPush.ItemId = dicItem?[mainDataItem.ItemCode].Id;
                    }
                    if (mainDataItem.TriggerType == TriggerMode.XHoursBefore.ToLabel())
                    {
                        prepareItemPush.TriggerType = TriggerMode.XHoursBefore;
                    }
                    else if (mainDataItem.TriggerType == TriggerMode.InvBelowSafeLevelToBeat.ToLabel())
                    {
                        prepareItemPush.TriggerType = TriggerMode.InvBelowSafeLevelToBeat;
                    }
                    else if (mainDataItem.TriggerType == TriggerMode.ManualModel.ToLabel())
                    {
                        prepareItemPush.TriggerType = TriggerMode.ManualModel;
                    }
                    if (prepareItemPush.ItemCategoryId.HasValue || prepareItemPush.ItemId.HasValue)
                    {
                        var push = RT.Service.Resolve<PrepareItemController>().GetPrepareItemPush(prepareItemPush.WipResourceId, prepareItemPush.ItemCategoryId, prepareItemPush.ItemId, prepareItemPush.TriggerType, mainDataItem.ItemExtPropName);
                        if (push != null)
                        {
                            throw new ValidationException("生产资源编码+物料类型+物料编码+物料扩展属性+触发方式存在重复;".L10N());
                        }
                    }
                    if (mainDataItem.ItemCode.IsNotEmpty())
                    {
                        ...
                        {
                            // 物料扩展验证
                            prepareItemPush.ItemExtProp = ExtPropIsActive(itemExtPropList, definitionList, catalogList, item, mainDataItem.ItemExtPropName, out itemExtPropErrorMsg);
                            prepareItemPush.ItemExtPropName = mainDataItem.ItemExtPropName;
                        }
                    ...
                    if (!mainDataItem.AdvanceHours.IsNullOrEmpty())
                    {
                        Decimal.TryParse(mainDataItem.AdvanceHours, out decimal advanceHours);
                        prepareItemPush.AdvanceHours = advanceHours;
                    }
                    if (!mainDataItem.SatisfactionTime.IsNullOrEmpty())
                    {
                        Decimal.TryParse(mainDataItem.SatisfactionTime, out decimal satisfactionTime);
                        prepareItemPush.SatisfactionTime = satisfactionTime;
                    }
                    if (mainDataItem.DemandType == DemandMode.WoSurplusQty.ToLabel())
                    {
                        prepareItemPush.DemandType = DemandMode.WoSurplusQty;
                    }
                    else if (mainDataItem.DemandType == DemandMode.StockToSafeLevelForBeat.ToLabel())
                    {
                        prepareItemPush.DemandType = DemandMode.StockToSafeLevelForBeat;
                    }
                    else if (mainDataItem.DemandType == DemandMode.StockIsSafeLevelForBeat.ToLabel())
                    {
                        prepareItemPush.DemandType = DemandMode.StockIsSafeLevelForBeat;
                    }
                    else if (mainDataItem.DemandType == DemandMode.FixedQuantity.ToLabel())
                    {
                        prepareItemPush.DemandType = DemandMode.FixedQuantity;
                    }
                    if (!mainDataItem.FixedQuantity.IsNullOrEmpty() && mainDataItem.DemandType == DemandMode.FixedQuantity.ToLabel())
                    {
                        Decimal.TryParse(mainDataItem.FixedQuantity, out decimal fixedQuantity);
                        prepareItemPush.FixedQuantity = fixedQuantity;
                    }
                    if (!mainDataItem.PreparationTime.IsNullOrEmpty() && (mainDataItem.DemandType == DemandMode.StockToSafeLevelForBeat.ToLabel() || mainDataItem.DemandType == DemandMode.StockIsSafeLevelForBeat.ToLabel()))
                    {
                        Decimal.TryParse(mainDataItem.PreparationTime, out decimal preparationTime);
                        prepareItemPush.PreparationTime = preparationTime;
                    }
                    prepareItemPush.PrepareItemType = PrepareItemType.Push;


                    RF.Save(prepareItemPush);
