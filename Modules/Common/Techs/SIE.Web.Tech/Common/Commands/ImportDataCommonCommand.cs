...
namespace SIE.Web.Tech.Common.Commands
{
    /// <summary>
    /// 通用数据导入命令
    /// </summary>
    public abstract class ImportDataCommonCommand : ImportCommandBase
    {
        ...
        protected override object ImportData(ImportViewArgs args, string sheetName = "", bool isFirstRowColumn = true)
        {
            ...
            ImportDataHandleMES importDataHandleExt = new ImportDataHandleMES(args.SelectedParentId);
    ...
    /// MES导入处理
    /// </summary>
    public class ImportDataHandleMES : ImportDataHandleExt
    {
        public ImportDataHandleMES(double parentId) : base(parentId)
        {
        }
        ...
        protected override bool ValidColumnNames(DataTable dt, List<string> columnList)
        {
            ...
            //{
            //    dt.Columns[i].ColumnName = columnList[i];
            //}
