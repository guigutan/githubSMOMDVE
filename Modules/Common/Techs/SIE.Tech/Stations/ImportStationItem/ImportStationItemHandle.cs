...
namespace SIE.Tech.Stations.ImportStationItem
{
    ...
    /// 导入工位物料 处理逻辑类
    /// </summary>
    [Services.Service(FallbackType = typeof(ImportStationItemHandle), ServiceLifeStyle = Services.ServiceLifeStyle.Transient)]
    public class ImportStationItemHandle : IDisposable, IBusinessImport
    {
        /// <summary>
        /// StationId的字段名称
        /// </summary>
        ...
        public void ProcessBusinessDataHandle(DataRow[] drs)
        {
            ...
            int sidIndex = drs[0].Table.Columns.IndexOf(StationId);
            foreach (var mainDataItem in drs)
            {
                ...
                var stationId = mainDataItem.Field<double>(sidIndex);
