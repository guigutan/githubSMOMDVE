...
(function (root, factory) {
    ...
    return function (config) {
        ...
        function escapeXmlChars(str) {
            if (typeof str == "string") return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&apos;').replace(/\r/g, '&#xD;').replace(/\n/g, '&#xA;'); else return str;
        }
        ...
        function checkInStdFiltersArrayForm(stdFiltersArrayForm, obj, name, path) {
            ...
            for (; idx < stdFiltersArrayForm.length; idx++) {
                ...
                } else if (_instanceof(filterPath, RegExp)) {
        ...
        function toArrayAccessForm(obj, childName, path) {
            switch (config.arrayAccessForm) {
                case "property":
                    if (!_instanceof(obj[childName], Array)) obj[childName + "_asArray"] = [obj[childName]]; else obj[childName + "_asArray"] = obj[childName];
                    break;
            ...
            if (!_instanceof(obj[childName], Array) && config.arrayAccessFormPaths.length > 0) {
        ...
        function parseDOMChildren(node, path) {
            if (node.nodeType == DOMNodeTypes.DOCUMENT_NODE) {
                    ...
                    if (child.nodeType != DOMNodeTypes.COMMENT_NODE) {
                        ...
                        if (checkXmlElementsFilter(result, child.nodeType, childName, childPath)) {
                            ...
                            if (result[childName] == null) {
                                ...
                                if (result[childName] != null) {
                                    if (!_instanceof(result[childName], Array)) {
                ...
                if (result["#text"] != null) {
                    ...
                    if (_instanceof(result.__text, Array)) {
        ...
        function jsonXmlSpecialElem(jsonObj, jsonObjField) {
            if (config.arrayAccessForm == "property" && endsWith(jsonObjField.toString(), "_asArray") || jsonObjField.toString().indexOf(config.attributePrefix) == 0 || jsonObjField.toString().indexOf("__") == 0 || _instanceof(jsonObj[jsonObjField], Function)) return true; else return false;
        }

        function jsonXmlElemCount(jsonObj) {
            var elementsCnt = 0;

            if (_instanceof(jsonObj, Object)) {
        ...
        function parseJSONTextObject(jsonTxtObj) {
            ...
            if (_instanceof(jsonTxtObj, Object)) {
        ...
        function parseJSONObject(jsonObj, jsonObjPath) {
            ...
            if (elementsCnt > 0) {
                for (var it in jsonObj) {
                    ...
                    } else if (_instanceof(subObj, Object)) {
                        if (_instanceof(subObj, Array)) {
                            result += parseJSONArray(subObj, it, attrList, jsonObjPath);
                        } else if (_instanceof(subObj, Date)) {
        ...
        this.asArray = function (prop) {
            if (prop === undefined || prop == null) return []; else if (_instanceof(prop, Array)) return prop; else return [prop];
        };

        this.toXmlDateTime = function (dt) {
            if (_instanceof(dt, Date)) return dt.toISOString(); else if (typeof dt === 'number') return new Date(dt).toISOString(); else return null;
        };
