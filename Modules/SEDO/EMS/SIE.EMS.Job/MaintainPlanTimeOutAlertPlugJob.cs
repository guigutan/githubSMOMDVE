...
namespace SIE.EMS.Job
{
    ...
    /// 保养计划超时预警
    /// </summary>
    [Job("保养计划超时预警", typeof(JobParameter))]
    public class MaintainPlanTimeOutAlertPlugJob : JobBase
    {
        /// <summary>
        /// 保养计划超时提醒
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            var alerterTypes = new List<Type> { typeof(MaintainPlanTimeOutAlertPlug) };
            ...
            if (alerters != null && alerters.Count > 0)
            {
                foreach (var curAlerter in alerters)
                {
                    try
                    {
                        ...
                        AddLog($"保养计划超时预警[{curAlerter.Name}]执行成功 !");
                    }
                    catch (Exception exMsg)
                    {
                        AddLog($"保养计划超时预警[{curAlerter.Name}]执行失败，错误信息: {exMsg.Message}");
                    }
                    finally
                    {
                        Thread.Sleep(10);
