SIE.defineCommand('SIE.Web.EMS.Purchases.EquipmentReceives.Commands.DeleteReceiveScanSnCommand', {
    extend: 'SIE.cmd.Delete',
    meta: { text: "删除", group: "edit", iconCls: "icon-DeleteEntity icon-red" },
    canExecute: function (view) {
        if (view.hasSelectedEntities()) {
            return true;
        }
        return false;
    },
    execute: function (view) {
        var msg = Ext.String.format('你确定删除选择的{0}条数据吗？'.t(), view.getSelection().length);
        SIE.Msg.askQuestion(msg, function () {
            view.removeSelection();
            var data = view.getData().data;
            if (data.length > 0) {
                view.getControl().setSelection(data.items[0]);
                view.setCurrent(data.items[0], true);
            } else {
                view.setCurrent(null, true);
            }
            let qty = view._parent.getCurrent().getOldRecivedQty();
            var line = view._parent.getCurrent().getReceiveLineNo();
            view.getData().data.items.forEach(function (p) {
                if (p.data.ReceiveLineNo === line) {
                    qty++;
                }
            });
            view._parent.getCurrent().setRecivedQty(qty);
        });
    }
});