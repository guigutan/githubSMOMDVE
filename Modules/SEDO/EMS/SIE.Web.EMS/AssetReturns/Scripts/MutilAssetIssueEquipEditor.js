Ext.define('SIE.Web.EMS.AssetReturns.Scripts.MutilAssetIssueEquipEditor', {
    extend: 'SIE.control.GridComboPopup',
    alias: 'widget.mutilIssueEquipEditor',
    setMULTIValue: function () {
        var me = this;
        var displayVal = "";
        me._targetSelectItems.items.forEach(function (model) {
            displayVal += me.separator + model.data.EquipAccountCode;
        });
        var entity;
        if (!me.up("form"))
        ...
        if (entity) {
            entity.setEquipmentCodes(displayVal.substr(1));
        }
    },
    /**
      * 创建界面布局
        ...
        SIE.AutoUI.getMeta({
            ...
            callback: function (blocks) {
                ...
                var filter = Ext.encode({
                    "Method": "GetAssetReturnEquipmentsById", "Parameters": [formEntity.data.Id, formEntity.data.AssetRequisitionId]
                });
                ui._view.loadData({
                    filter: filter,
                    action: 'queryer',
