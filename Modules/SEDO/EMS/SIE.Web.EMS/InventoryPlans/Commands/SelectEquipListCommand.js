SIE.defineCommand('SIE.Web.EMS.InventoryPlans.Commands.SelectEquipListCommand', {
    extend: 'SIE.cmd.LookupCommandBase',
    meta: { text: "选择设备台账", group: "edit", iconCls: "icon-PlaylistCheck icon-blue" },
    userConfig: {
        dataParams: {
            specKeyPrototyName: 'EquipAccoutId',
            ...
            SIE.each(selections.items, function (sel) {
                ...
                if (me._sourceViewSelectItems.indexOf(id) === -1) {
                    var item = {
                        ...
                        EquipAccoutId: id,
                        EquipModelId: sel.getEquipModelId(),
                        EquipTypeId: sel.getEquipTypeViewId(),
                        UseDeptId: sel.getUseDepartmentId(),
                        WorkShopId: sel.getWorkShopId()
                    };
                    items.push(item);
                }
            });
        ...
        if (me._targetView !== null) {
            ...
            if (store !== null) {
                ...
                if (dialogView._relations[0]) { //存在查询面板时
                    ...
                    //查询实体赋值
                    var factoryId = me.view.getParent().getCurrent().getFactoryId();
                    criteria.setFactoryId(factoryId);
                    criteria.setIsSelect(true);
                    criteria.setIsLoadAll(true);
