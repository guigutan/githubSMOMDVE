SIE.defineCommand('SIE.Web.EMS.FixedAssets.Accounts.Commands.SelFixedAssetFixtureCommand', {
    ...
    userConfig: {
        dataParams: {
            specKeyPrototyName: 'FixtureIDAccountId',
            targetClassName: 'SIE.Fixtures.Fixtures.Accounts.FixtureIDAccount'
        }
    },
        ...
        if (selections && selections.length > 0) {
            selections.forEach(function (sel) {
                if (me._sourceViewSelectItems.indexOf(sel.getId()) === -1) {
                    ...
                    data.setFixtureIDAccountId(sel.data.Id);
                    data.setCode(sel.data.Code);
                    data.setEncodeCode(sel.data.EncodeCode);
                    data.setAccountState(sel.data.AccountState);
                    data.setModelCode(sel.data.ModelCode);
                    data.setModelName(sel.data.ModelName);
                    data.setFixtureTypeCode(sel.data.FixtureTypeCode);
                    data.setUnitName(sel.data.UnitName);
                    data.setWarehouseName(sel.data.WarehouseName);
                    data.setStorageLocationName(sel.data.LocationName);
        ...
        if (e.property == 'IsMajor') {
            ...
            if (e.entity.data.IsMajor) {
                ...
                for (var i = 0; i < e.entity.store.getCount(); i++) {
                    ...
                    if (e.entity.data.FixtureIDAccountId != record.data.FixtureIDAccountId) {
