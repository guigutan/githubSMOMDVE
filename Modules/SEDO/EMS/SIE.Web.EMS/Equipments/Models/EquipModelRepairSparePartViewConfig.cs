...
namespace SIE.Web.EMS.Equipments.Models
{
    ...
    /// 备件视图
    /// </summary>
    public class EquipModelRepairSparePartViewConfig : WebViewConfig<EquipModelRepairSparePart>
    {
        /// <summary>
        /// 配置列表
        /// </summary>
        protected override void ConfigListView()
        {
            ...
            using (View.OrderProperties())
            {
                ...
                {
                    var item = source as EquipModelRepairSparePart;
                    if (item != null)
                    {
                ...
                }).UsePagingLookUpEditor((m, e) =>
                {
                    ...
                    m.DicLinkField = dic;
                }).HasLabel("备件编码").ShowInList(20 * 4);
                View.Property(p => p.SparePartName).HasLabel("备件名称").Readonly().ShowInList(20 * 5).Readonly();
                View.Property(p => p.Specification).HasLabel("型号规格").Readonly().ShowInList(20 * 5).Readonly();
                View.Property(p => p.UnitName).Readonly().ShowInList(20 * 4);
                View.Property(p => p.Qty).UseSpinEditor(p =>
                {
                    p.AllowNegative = false;
                    p.AllowDecimals = false;
                    p.MinValue = 1;
                }).DefaultValue(1).ShowInList(20 * 3);
