Ext.define('SIE.Web.EMS.Equipments.Scripts.EquipAccountListViewBehavior',
    {
        onDataLoaded: function (view) {
            if (view && view.getCommands().length > 0) {//view.getCommands().length>0 排除弹出层
                var count = view.getData() != null ? view.getData().data.length : 0;
                var ieExsitedEl = Ext.fly("equipcountId");
                if (!ieExsitedEl) {
                    var el = {
                        tag: 'div',
                        id: 'equipcountId',
                        style: 'z-index:99;float:right;top:10px;margin-right:25px;',
                        cls: 'x-box-inner',
                        html: '查询设备数:'.t() + count + '台'.t()
                    };
                    Ext.DomHelper.insertAfter(view.getControl().ownerCt.el.id, el);
                } else
                    ieExsitedEl.dom.innerText = '查询设备数:'.t() + count + '台'.t();
            }
        },
        ...
        */
        ...
        rowclick: function (g, record, element, rowIndex, e, eOpts) {
            ...
            var equipAccountCheckMgtViewConfig = null;
            if (tabs) {
                var equipAccountCheckMgtViewConfig= tabs.first(function (e) { return e.viewConfig === "SIE.Web.EMS.Equipments.Accounts.EquipAccountCheckMgtViewConfig"; })

            }
            var tabPanel = equipAccountCheckMgtViewConfig != null ? equipAccountCheckMgtViewConfig.getControl().ownerCt.ownerCt.ownerCt.ownerCt : null;
