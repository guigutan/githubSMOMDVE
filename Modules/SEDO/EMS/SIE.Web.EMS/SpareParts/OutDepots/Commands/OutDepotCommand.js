SIE.defineCommand('SIE.Web.EMS.SpareParts.OutDepots.Commands.OutDepotCommand', {
    extend: 'SIE.cmd.FormSave',
    meta: { text: "出库", group: "edit", iconCls: "icon-TextRelease icon-blue" },
    canExecute: function (view) {
        if (view.getCurrent() == null) {
            return false;
        }
        var state = view.getCurrent().getOutDepotState();
        if (state == 0) {
            return true;
        }
    ...
    execute: function (view, source) {
        var me = this;
        SIE.Msg.askQuestion("是否出库？提交后单据不能修改。".t(),
            function () {
    ...
    onSaved: function (view, res) {
        ...
        SIE.Msg.showInstantMessage('出库成功！');
    }
});