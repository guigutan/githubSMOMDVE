...
namespace SIE.ERPInterface.Job.Upload
{
    ...
    /// 供应商退货事务上传
    /// </summary>
    [Job("暂收供应商退货事务上传EBS", typeof(JobParameter))]
    public class UploadReceiveSupplierReturnJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            {
                AppRuntime.InvOrg = context.InvOrg;
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.SupplierReturn, TransactionType.OutStorage, null, ""));       //没有配置规则，默认事务定义
                RT.Service.Resolve<UploadBaseController>().UploadInvTransactionsToInf(tuples);                          //捉取事务交易数据到事务上传表
                AddLog("供应商退货结束上传中间表。".L10N());
            }

            AppRuntime.InvOrg = context.InvOrg;
            var resultSmom = RT.Service.Resolve<EbsPoReceiveSupplierReturnController>().UploadToEbs(tuples);
            AddLog("采购暂收供应商退货结束上传ERP。{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));
        }
