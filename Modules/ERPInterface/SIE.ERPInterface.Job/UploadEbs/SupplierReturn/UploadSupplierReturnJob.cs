...
namespace SIE.ERPInterface.Job.Upload
{
    ...
    /// 供应商退货事务上传
    /// </summary>
    [Job("供应商退货事务上传EBS", typeof(JobParameter))]
    public class UploadSupplierReturnJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            AppRuntime.InvOrg = context.InvOrg;
            var resultSmom = RT.Service.Resolve<EbsSupplierReturnController>().UploadToEbs(tuples);
            AddLog("供应商退货结束上传ERP。{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));            
        }
