...
using SIE.ERPInterface.Ebs.UploadEbs.OtherOut;
using SIE.ERPInterface.Ebs.UploadEbs.SupplierReturn;
...
namespace SIE.ERPInterface.Job.Upload
{
    ...
    /// 其他出入库事务上传
    /// </summary>
    [Job("其他出入库事务上传", typeof(JobParameter))]
    public class UploadOtherJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            {
                AppRuntime.InvOrg = context.InvOrg;
                //tuples.Add(new Tuple<OrderType, TransactionType>(OrderType.OtherIn, TransactionType.InStorage));
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.OtherOut, TransactionType.OutStorage,null,""));       //没有配置规则，默认事务定义
                RT.Service.Resolve<UploadBaseController>().UploadInvTransactionsToInf(tuples);                          //捉取事务交易数据到事务上传表
                AddLog("其他出入库结束上传中间表。".L10N());
            }
            AppRuntime.InvOrg = context.InvOrg;
            var resultSmom = RT.Service.Resolve<EbsOtherOutController>().UploadToEbs(tuples);
            AddLog("其他出入库结束上传ERP。{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));
        }
