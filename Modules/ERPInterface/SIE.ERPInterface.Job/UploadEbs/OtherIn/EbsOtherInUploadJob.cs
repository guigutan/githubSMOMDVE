...
using SIE.ERPInterface.Ebs.UploadEbs.OtherIn;
...
namespace SIE.ERPInterface.Job.Upload.OtherIn
{
    ...
    /// 其他入库事务上传
    /// </summary>
    [Job("其他入库事务上传EBS", typeof(JobParameter))]
    public class EbsOtherInUploadJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            {
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.OtherIn, TransactionType.InStorage, null, ""));      //没有配置规则，默认事务定义
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.OtherIn, TransactionType.RecInStorage, null, ""));       //没有配置规则，默认事务定义
                RT.Service.Resolve<UploadBaseController>().UploadInvTransactionsToInf(tuples);                          //捉取事务交易数据到事务上传表
                AddLog("其他入库结束上传中间表。".L10N());
            }
            AppRuntime.InvOrg = context.InvOrg;
            //Copy必改内容
            var resultSmom = RT.Service.Resolve<EbsOtherInController>().UploadToEbs(tuples);         //执行数据上传ERP
            AddLog("其他入库结束上传ERP。{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));
        }
