...
using SIE.ERPInterface.Ebs.UploadEbs.SaleReturn;
...
namespace SIE.ERPInterface.Job.Upload.SaleReturn
{
    ...
    /// 销售退货事务上传
    /// </summary>
    [Job("销售退货事务上传EBS", typeof(JobParameter))]
    public class EbsUploadSaleReturnJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            else
            {
                ...
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.SaleReturn, TransactionType.RecInStorage, null, ""));
                RT.Service.Resolve<UploadBaseController>().UploadInvTransactionsToInf(tuples);                          //捉取事务交易数据到事务上传表
                AddLog("销售退货结束上传中间表。".L10N());
            }
            //Copy必改内容
            var resultSmom = RT.Service.Resolve<EbsSaleReturnController>().UploadToEbs(tuples);         //执行数据上传ERP
            AddLog("销售退货结束上传ERP。{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));
        }
