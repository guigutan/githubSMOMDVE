...
namespace SIE.ERPInterface.Job.Upload.Finished
{
    ...
    /// 成品/半成品入库事务上传
    /// </summary>
    [Job("成品入库事务上传EBS", typeof(JobParameter))]
    public class EbsUploadFinishedJob : JobBase
    {
        ///// <summary>
        ///// 执行调度
        ///// </summary>
        ...
        //    {
        //        //成品入库
        //        tuples.Add(new Tuple<OrderType, TransactionType>(OrderType.Finished, TransactionType.InStorage));//没有配置规则，默认事务定义
        //        tuples.Add(new Tuple<OrderType, TransactionType>(OrderType.Finished, TransactionType.RecInStorage));//没有配置规则，默认事务定义
        //        //半成品入库
        //        //tuples.Add(new Tuple<OrderType, TransactionType>(OrderType.PartedIn, TransactionType.InStorage));//没有配置规则，默认事务定义
        //        RT.Service.Resolve<UploadBaseController>().UploadInvTransactionsToInf(tuples);//捉取事务交易数据到事务上传表
        //        AddLog("成品入库结束上传中间表。".L10N());
        //    }
        //    AppRuntime.InvOrg = context.InvOrg;
        //    //Copy必改内容
        //    var resultSmom = RT.Service.Resolve<EbsFinishedInController>().UploadToEbs(tuples, OrderType.Finished);//执行数据上传ERP
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                //成品入库
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.Finished, TransactionType.InStorage, null, ""));//没有配置规则，默认事务定义
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.Finished, TransactionType.RecInStorage, null, ""));//没有配置规则，默认事务定义
                //半成品入库
                //tuples.Add(new Tuple<OrderType, TransactionType>(OrderType.PartedIn, TransactionType.InStorage));//没有配置规则，默认事务定义
                RT.Service.Resolve<UploadBaseController>().UploadInvTransactionsToInf(tuples);//捉取事务交易数据到事务上传表
                AddLog("成品入库结束上传中间表。".L10N());
