...
namespace SIE.ERPInterface.Job.Download.Receipts
{
    ...
    /// ASN下载
    /// </summary>
    [Job("ASN下载", typeof(DLCommonParameter))]
    public class DownloadAsnJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<SoapAsnController>().DownloadToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadAsnController>().DownloadAsnInfToBusiness();           //执行业务表下载
