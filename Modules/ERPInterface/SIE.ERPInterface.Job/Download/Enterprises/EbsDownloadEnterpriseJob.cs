...
using SIE.ERPInterface.Ebs.Download.Enterprises;
using System;
...
namespace SIE.ERPInterface.Job.Download.Projects
{
    ...
    /// EBS企业模型下载
    /// </summary>
    [JobAttribute("6.EBS企业模型下载", typeof(JobParameter))]
    ...
    {        
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            var resultSmom = RT.Service.Resolve<EbsEnterpriseController>().Download(context.InvOrg);           //执行业务表下载
            AddLog("企业模型数据结束下载{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));
        }
