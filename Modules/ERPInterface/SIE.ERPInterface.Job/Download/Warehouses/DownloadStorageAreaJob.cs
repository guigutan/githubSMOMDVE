...
namespace SIE.ERPInterface.Job.Download.Warehouses
{
    ...
    /// 库区下载
    /// </summary>
    [Job("库区下载", typeof(DLCommonParameter))]
    public class DownloadStorageAreaJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<SoapWarehouseController>().DownloadStorageAreaERPToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadStorageAreaController>().DownloadStorageAreaInfToBusiness();           //执行业务表下载
