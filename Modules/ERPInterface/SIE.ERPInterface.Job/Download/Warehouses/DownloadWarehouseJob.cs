...
namespace SIE.ERPInterface.Job.Download.Warehouses
{
    ...
    /// 仓库下载
    /// </summary>
    [Job("仓库下载", typeof(DLCommonParameter))]
    public class DownloadWoJob : JobBase
    {
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<SoapWarehouseController>().DownloadToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadWarehouseController>().DownloadWarehouseInfToBusiness();           //执行业务表下载
