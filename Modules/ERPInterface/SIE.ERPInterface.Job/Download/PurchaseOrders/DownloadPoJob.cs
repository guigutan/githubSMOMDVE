...
namespace SIE.ERPInterface.Job.Download.PurchaseOrders
{
    /// <summary>
    /// 采购订单下载
    /// </summary>
    [Job("采购订单下载", typeof(DLCommonParameter))]
    public class DownloadPoJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<DbPoController>().DownloadToInf();//执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadPoController>().DownloadPoInfToBusiness();           //执行业务表下载
