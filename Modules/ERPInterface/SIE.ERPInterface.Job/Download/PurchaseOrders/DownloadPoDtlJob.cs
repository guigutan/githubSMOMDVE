...
namespace SIE.ERPInterface.Job.Download.Items
{
    ...
    /// 采购订单明细下载
    /// </summary>
    [Job("采购订单明细下载", typeof(DLCommonParameter))]
    public class DownloadPoDtlJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<DbPoDtlController>().DownloadToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadPoDtlController>().DownloadPoDtlInfToBusiness();           //执行业务表下载
