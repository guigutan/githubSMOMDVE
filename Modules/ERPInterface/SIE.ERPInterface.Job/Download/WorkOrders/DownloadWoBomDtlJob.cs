...
namespace SIE.ERPInterface.Job.Download.WorkOrders
{
    ...
    /// 工单Bom下载
    /// </summary>
    [Job("工单Bom下载", typeof(DLCommonParameter))]
    public class DownloadWoBomDtlJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<SoapWorkOrderController>().DownloadWorkOrderBomERPToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadWorkOrderBomController>().DownloadWorkOrderBomInfToBusiness();           //执行业务表下载
