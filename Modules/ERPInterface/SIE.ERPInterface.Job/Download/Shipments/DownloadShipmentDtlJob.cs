...
namespace SIE.ERPInterface.Job.Download.Shipments
{
    ...
    /// 发运单明细下载
    /// </summary>
    [Job("发运单明细下载", typeof(DLCommonParameter))]
    public class DownloadShipmentDtlJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<SoapShipmentController>().DownloadShipmentDtlERPToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadShipmentDtlController>().DownloadOrderDtlInfToBusiness();           //执行业务表下载
