...
namespace SIE.ERPInterface.Job.Download.Customers
{
    ...
    /// 客户下载
    /// </summary>
    [Job("客户下载", typeof(DLCommonParameter))]
    public class DownloadCustomerJob : JobBase
    {
        /// <summary>
        /// 执行调度
        /// </summary>
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var resultInf = RT.Service.Resolve<SoapCustomerController>().DownloadToInf();                     //执行中间表下载
                AddLog("中间表结束下载{0}".L10nFormat(resultInf == null ? "。" : "，" + resultInf.Msg));
            }

            var resultSmom = RT.Service.Resolve<DownloadCustomerController>().DownloadCustomerInfToBusiness();           //执行业务表下载
