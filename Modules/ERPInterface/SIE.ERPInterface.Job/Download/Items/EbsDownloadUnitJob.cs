...
namespace SIE.ERPInterface.Job.Download.Items
{
    ...
    /// EBS单位下载
    /// </summary>
    [JobAttribute("3.EBS单位下载", typeof(JobParameter))]
    public class EbsDownloadUnitJob : JobBase
    {
        //
        // 摘要:
        //     执行任务
        ...
        protected override void ExecuteJob(JobContext context)
        {
            ...
            var resultSmom = RT.Service.Resolve<EbsUnitController>().Download(context.InvOrg);           //执行业务表下载
            AddLog("物料单位基础表结束下载{0}".L10nFormat(resultSmom == null ? "。" : "，" + resultSmom.Msg));
        }
