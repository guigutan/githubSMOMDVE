...
namespace SIE.ERPInterface.Job.UploadSap.OutboundConfirm
{
    /// <summary>
    /// 发货确认上传调度
    /// </summary>
    [Job("发货确认上传调度", typeof(JobParameter))]
    public class OutboundConfirmUploadJob: JobBase
    {
        protected override void ExecuteJob(object param)
        {
            var redisKey = "OutboundConfirmUploadJob" + RT.InvOrg;
            string lockId = null;
            ...
            try
            {
                ...
                RT.Service.Resolve<UploadBaseController>().OutboundConfirmTransaction();
                AddLog("结束上传中间表。".L10N());

                //上传ERP
                var result = RT.Service.Resolve<HttpSapOutboundConfirmController>().UploadOutboundConfirm();
