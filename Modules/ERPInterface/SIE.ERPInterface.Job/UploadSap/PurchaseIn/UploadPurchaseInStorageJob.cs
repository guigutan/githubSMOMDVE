...
namespace SIE.ERPInterface.Job.Upload.PurchaseIn
{
    ...
    /// 采购入库上传调度
    /// </summary>
    [Job("采购入库上传调度", typeof(ULCommonParameter))]
    public class UploadPurchaseInStorageJob : JobBase
    {
        /// <summary>
        /// 调度执行主键
        /// </summary>
        public static string _jobKey = "采购入库-" + typeof(UploadPurchaseInStorageJob).FullName + "!" + RT.InvOrg;
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                //没有配置规则，默认事务定义
                var transaction = RT.Service.Resolve<TransactionController>().GetTransactionByCodes( new List<string>() { "PurchaseIn" });
                if (transaction == null|| !transaction.Any())
                    throw new ValidationException("未找到[采购入库]的单据小类，请维护。".L10N());
               
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.PurchaseIn, TransactionType.InStorage, transaction.FirstOrDefault().Id, string.Empty));
                tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.PurchaseIn, TransactionType.RecInStorage, transaction.FirstOrDefault().Id, string.Empty));
                //捉取事务交易数据到事务上传表
            ...
            if (tuples.Count > 0)
            {
                ...
                //上传ERP
                var result = RT.Service.Resolve<HttpSapPurchaseInController>().UploadInStorageToErp(uploadTuples);
