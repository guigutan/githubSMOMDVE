...
namespace SIE.ERPInterface.Job.Upload
{
    ...
    /// 委外出库事务上传
    /// </summary>
    [Job("委外出库事务上传", typeof(ULCommonParameter))]
    public class UploadOutPurchase : JobBase
    {
        /// <summary>
        /// 调度执行主键
        /// </summary>
        public static string _jobKey = "委外出库事务上传-" + typeof(UploadOutPurchase).FullName + "!" + RT.InvOrg;
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var transactions = RT.Service.Resolve<TransactionController>().GetTransactions(OrderType.OutWorkFeed);
                foreach (var item in transactions)
                {
                    tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.OutWorkFeed, TransactionType.OutStorage, item.Id, ""));
                }
                //没有配置规则，默认事务定义
            ...
            { //单据大类，事务类型，单据小类，单据号
                ...
                var result = RT.Service.Resolve<HttpOutPurchaseController>().UploadPurchaseOutToErp(uploadTuples);                      //上传ERP
