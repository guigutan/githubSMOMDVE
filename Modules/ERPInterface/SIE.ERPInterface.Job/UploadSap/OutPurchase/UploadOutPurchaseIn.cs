...
namespace SIE.ERPInterface.Job.Upload
{
    ...
    /// 委外入库事务上传
    /// </summary>
    [Job("委外入库事务上传", typeof(ULCommonParameter))]
    public class UploadOutPurchaseIn : JobBase
    {
        /// <summary>
        /// 调度执行主键
        /// </summary>
        public static string _jobKey = "委外入库事务上传-" + typeof(UploadOutPurchaseIn).FullName + "!" + RT.InvOrg;
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var transactions = RT.Service.Resolve<TransactionController>().GetTransactions(OrderType.OutMaterialReturn);
                foreach (var item in transactions)
                {
                    tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.OutMaterialReturn, TransactionType.InStorage, item.Id, ""));
                    tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.OutMaterialReturn, TransactionType.RecInStorage, item.Id, ""));
                }
                //没有配置规则，默认事务定义
            ...
            { //单据大类，事务类型，单据小类，单据号
                ...
                var result = RT.Service.Resolve<HttpOutPurchaseController>().UploadPurchaseInToErp(uploadTuples);                      //上传ERP
