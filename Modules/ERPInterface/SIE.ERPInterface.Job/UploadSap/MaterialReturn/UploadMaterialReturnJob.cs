...
using SIE.ERPInterface.Sap.Upload.MaterialReturn;
...
namespace SIE.ERPInterface.Job.Upload
{
    ...
    /// 生产退料事务上传
    /// </summary>
    [Job("生产退料事务上传", typeof(JobParameter))]
    public class SapUploadMaterialReturnJob : JobBase
    {
        /// <summary>
        /// 调度执行主键
        /// </summary>
        public static string _jobKey = "生产退料事务上传-" + typeof(UploadMaterialReturnJob).FullName + "!" + RT.InvOrg;
        ...
        protected override void ExecuteJob(object param)
        {
            ...
            {
                var transactions = RT.Service.Resolve<TransactionController>().GetTransactions(OrderType.MaterialReturn);
                foreach (var item in transactions)
                {
                    tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.MaterialReturn, TransactionType.InStorage, item.Id, ""));
                    tuples.Add(new Tuple<OrderType, TransactionType, double?, string>(OrderType.MaterialReturn, TransactionType.RecInStorage, item.Id, ""));
                }
                //没有配置规则，默认事务定义
            ...
            { //单据大类，事务类型，单据小类，单据号
                ...
                var result = RT.Service.Resolve<HttpMaterialReturnController>().UploadMaterialReturnToErp(uploadTuples);                      //上传ERP
