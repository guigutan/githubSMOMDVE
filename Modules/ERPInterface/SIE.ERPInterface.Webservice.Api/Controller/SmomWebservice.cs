...
namespace SIE.ERPInterface.Webservice.Api.Controller
{
    ...
    public class SmomWebservice
    {
        ...
        /// 保存物料数据
        /// </summary>
        /// <param name="unitDatas">物料数据</param>
        /// <param name="invOrg">组织ID，大于0有效</param>
        /// <returns></returns>
        [OperationContract]
        public ApiResult SaveItems(List<UnitData> unitDatas, int invOrg = 0)
        {
            var ctl = RT.Service.Resolve<SmomWebApiController>();
            return ctl.SaveUnits(unitDatas, invOrg);
        }
        ...
        /// <returns></returns>
        [OperationContract]
        public ApiResult SaveItems(List<ItemData> itemDatas, int invOrg = 0)
        {
            var ctl = RT.Service.Resolve<SmomWebApiController>();
