...
namespace SIE.ERPInterface.Ebs.UploadEbs.SupplierReturn
{
    ...
    /// 采购暂收退货供应商控制器
    /// </summary>
    public class EbsPoReceiveSupplierReturnController : EbsUploadBaseController
    {
        /// <summary>
        /// 上传数据到EBS
        /// </summary>
        ...
        public virtual ProcessResult UploadToEbs(List<Tuple<OrderType, TransactionType, double?, string>> tuples)
        {
            ...
            // 接口编码
            ebsPara.InterfaceCode = "S_W2E_PO_RETURN_RCV";
            ...
            uploadTransactions.Where(p => p.OrderType == ebsPara.OrderType && (p.Transaction.Code == "ReceiveReturn") &&
              p.PoErpKey.IsNotEmpty() && p.PoLineErpKey.IsNotEmpty()).ForEach(f =>
            {
            ...
            ""UOM_CODE"": ""{11}"",
            ""RECEIPT_NUM"": ""{12}""
            }".FormatArgs(item.AsnNo, item.AsnLineNo, item.TranId, item.OrgName, item.OrganizationName, item.PoErpKey, item.PoLineErpKey,
            item.TransactionType, item.TransactionDate, item.ItemCode, item.Quantity, item.UnitCode, item.AsnNo);
