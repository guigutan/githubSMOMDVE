...
using SIE.ERPInterface.Sap.Datas.ErpInfoDatas.ScrapWeighingRecord;
...
{
    public class UploadScrapWeighingHandle : IUploadDataHandler<KzScrapWeighingUploadData>
    {
        ...
        public SapUploadParam<KzScrapWeighingUploadData> SetUploadData(List<UploadTransaction> uploadTransactions)
        {
        ...
        {
            var param = JsonConvert.DeserializeObject<SapResponseParam<KzScrapWeighingResultData>>(sapResult.ResponseStr);
            ...
            foreach (var r in param.Return)
            {
                if (r.ZFKBS == "S")
                {
                    ...
                    //    if (sourIds.Count > 0)
                    //        DB.Update<SIE.MES.TaskManagement.FeedingRecords.ScrapWeighingRecord>().Set(p => p.UploadResult, r.ZFKXX).Set(p => p.Mblnr, r.MBLNR).Set(p => p.Mjahr, r.MJAHR).Where(p => sourIds.Contains(p.Id)).Execute();
                    //}
