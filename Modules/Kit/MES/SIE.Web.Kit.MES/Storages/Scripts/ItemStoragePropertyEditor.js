Ext.define('SIE.Web.Kit.MES.Storages.ItemStoragePropertyEditor', {
    ...
    onClick: function (field, trigger, e) {
        ...
        if (cur.phantom == true) {
            SIE.Msg.showError("当前货位物料明细未保存，请先保存再选择物料属性值！".t());
            return false;
        }
        if (!itemId) {
        ...
        SIE.AutoUI.getMeta({
            model: 'SIE.Kit.MES.Storages.ItemStoragePropertyValue',
            ...
            callback: function (res) {
                ...
                var win = SIE.Window.show({
                    ...
                    callback: function (btn) {
                        if (btn == "确定".t()) {
                            ...
                            var savePropertyValue = Ext.create('SIE.Web.Kit.MES.Storages.Commands.ItemStoragePropertySaveCommand');
                            savePropertyValue.execute(listView, {
                                Selecteds: selecteds,
                                DetailId: detailId,
                            }, win);
                            CRT.Event.fire('SIE.Kit.MES.Storages.StorageLocation_refresh');
                            return false;
                        }
                    }
                });
                ...
                var filter = {
                    Method: 'GetItemStoragePropertyList',
