...
namespace SIE.KZ.Group.SmomControl.Jobs
{
    ...
    /// 总控向子工厂下发工单数据
    /// </summary>
    [Job("总控向子工厂下发工单数据", typeof(JobParameter))]

    public class SendGroupWorkOrderDataToFactoryJob : JobBase
    {
        //是否运行中,用来防止并发问题
        ...
        {
            if (SendGroupWorkOrderDataToFactoryJob.IsRun == true)
                throw new ValidationException("任务正在运行中,不允许并发执行".L10N());
            SendGroupWorkOrderDataToFactoryJob.IsRun = true;
