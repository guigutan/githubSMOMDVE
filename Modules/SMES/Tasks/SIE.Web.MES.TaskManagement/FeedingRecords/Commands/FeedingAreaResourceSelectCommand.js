SIE.defineCommand('SIE.Web.MES.TaskManagement.FeedingRecords.Commands.FeedingAreaResourceSelectCommand', {
        ...
        if (selections && selections.items.length > 0) {
            ...
            SIE.each(selections.items, function (sel) {
                ...
                if (me._sourceViewSelectItems.indexOf(id) === -1) {
                    var item = { FeedingAreaId: me._sourceId, ResourceId: id };
                    items.push(item);
                }
            });
            if (items.length > 0) {
