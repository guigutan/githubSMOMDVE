...
namespace SIE.Web.MES.TaskManagement.Completion
{
    ...
    /// 视图配置
    /// </summary>
    internal class ProductCompletionViewConfig : WebViewConfig<ProductCompletion>
    {
        protected override void ConfigListView()
        {
            ...
            using (View.OrderProperties())
            {
                ...
                View.Property(p => p.TaskQty).ShowInList().Readonly().HasLabel("计划数量");
                ...
                View.Property(p => p.ShiftCapacity).ShowInList().Readonly();
                View.Property(p => p.CapacityLoad).ShowInList().Readonly()
                    .UseDisplayEditor(p => p.ColumnXType = "CapacityLoadColorChange")
                    .UseListSetting(e => { e.HelpInfo = "产能负荷（%）= 计划数量/(班次产能*班次数量)"; });
                //View.Property(p => p.Shift).ShowInList(150).Readonly();
                ...
                View.AttachChildrenProperty(typeof(ProductCompletionDetail), (e) =>
                {
                    var args = e as ChildPagingDataWithParentEntityArgs;
                    var entity = args.ParentEntity.ToJsonObject<ProductCompletion>();
                    return RT.Service.Resolve<ProductCompletionController>().GetProductCompletionDetails(entity);
                });
            }
        }
    }
