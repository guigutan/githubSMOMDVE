SIE.defineCommand('SIE.Web.Andon.Andons.Commands.AndonTypeTriggerPowerAddUserGroupCommand', {
    extend: 'SIE.cmd.Add',
    meta: { text: "添加用户组", group: "edit", iconCls: "icon-AddEntity icon-green" },
    showView: function (editEntity) {
        ...
        if (!this.viewMeta) {
            SIE.AutoUI.getMeta({
                model: "SIE.Rbac.Users.UserGroup",
                ignoreCommands: true,
                isDetail: false,
                ignoreQuery: true,
                viewGroup: "SelectUserGroupView",
                callback: function (res) {
                    ...
                    SIE.invokeDataQuery({
                        ...
                        method: "GetUserGroupAlternative",
                        ...
                        listeners: {
                            specialKey: function (field, e) {
                                if (e.getKey() == Ext.EventObject.ENTER) {
                                    ...
                                    var searchTxt = this.ownerCt.child("[name = usergroup_searchTxt]");
                                    var Filter = {
                                        Method: "GetUserGroupAll",
                            ...
                            items: [{
                                ...
                                handler: function () {
                                    ...
                                    if (selectList.length > 10) {
                                        var msg = '一次选择添加的用户组数不能超过10！'.L10N();
                                    ...
                                    for (var i = 0; i < selectList.length; i++) {
                                        ...
                                        if (index >= 0) {
                                            var msg = '用户组['.L10N() + selectList[i].data.Code + ']已添加,不允许重复添加!'.L10N();
                                            SIE.Msg.showMessage(msg);
                                            return
                                        }
                                    }
                                ...
                                handler: function () {
                                    ...
                                    if (selectList.length > 10) {
                                        var msg = '一次选择移除的用户组数不能超过10！'.L10N();
                                        SIE.Msg.showMessage(msg);
                                        return
                                    }
                                    selectList.forEach(function (item) {
