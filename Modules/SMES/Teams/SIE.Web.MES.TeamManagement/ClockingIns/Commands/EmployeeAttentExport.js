SIE.defineCommand('SIE.Web.MES.TeamManagement.ClockingIns.Commands.EmployeeAttentExport', {
    ...
    execute: function (view) { 
        var me = this;
        me._view = view;
        var queryView = CRT.Context.PageContext.getQueryView();//todo 框架提供更便捷的获取查询参数的方法
        if (queryView) {
            var criter = queryView.getCurrent();
    ...
    generateExcel: function (exportData) { 
        SIE.Web.MES.Common.Scripts.Helpers.ExportExcelHelper.tablesToMultiSheetExcel(exportData, '人员工时统计'.L10N() + Ext.util.Format.date(new Date(), 'Ymdhis'), true);
    },
    ...
    getstoreData: function (token) {
        ...
        SIE.invokeDataQuery({
            ...
            success: function (res) {
                ...
                me.ctYear.setValue(res.Result[0]['Year']);//todo null ref
