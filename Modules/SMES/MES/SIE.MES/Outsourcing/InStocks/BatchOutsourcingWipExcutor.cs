...
namespace SIE.MES.Outsourcing.InStocks
{
    ...
    /// 工序委外在制控制器
    /// </summary>
    public class BatchOutsourcingWipExcutor
    {
        /// <summary>
        /// 运行时控制器
        /// </summary>
        ...
        /// 生产产品版本列表
        /// </summary>
        private readonly IList<BatchWipProductVersion> wipProductVersions;
        ...
        public BatchOutsourcingWipExcutor(IList<BatchWipProductVersion> wipProductVersions)
        {
            ...
            dateTime = RF.Find<BatchWipProductVersion>().GetDbTime();
        ...
        {
            const RetrospectType retrospectType = RetrospectType.Batch;
            ...
            if (lastOutsouringProcess.IsEnd)
            {
                ...
                {
                    var sn = wipProductVersion.BatchNo;
                    ...
                    RT.EventBus.Publish(new WipFinishedEvent(product, sn, dateTime, null));
                    ...
                    if (!workOrderBoms.Any())
                    {
                        var costItems = RT.Service.Resolve<BackflushMaterialController>().
                            CreateDeductItems(sn, wo.ResourceId, lastOutsouringProcess.ProcessId, stationId: null,
                                wo.FactoryId.Value, product.WorkOrderId, product.Qty, workOrderBoms, retrospectType);
        ...
        private void GoToRoutingProccess(EntityList<ProductEntity> productEntities,
            string sn, product product, double fromProcessId, process lastOutsouringProcess, BatchWipProductVersion wipProductVersion)
        {
        ...
        private void GetProducts(IList<BatchWipProductVersion> wipProductVersions, EntityList<PuidMap> puidMaps,
        ...
        private string CreatePuidKey(BatchWipProductVersion x)
        {
            return runtimeController.CreatePuidKey(x.BatchNo, BarcodeType.BatchBarocde);
        }
    }
}
