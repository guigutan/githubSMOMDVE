...
namespace SIE.MES.ForWinform
{
    ...
    ///XP维修API控制器
    /// </summary>
    public class WinFormTemporaryRepairApiController : WipTemporaryRepairController
    {
        /// <summary>
        /// 获取当前条码最后的生产版本
        /// </summary>
        ...
        /// <returns></returns>
        [ApiService("获取当前条码最后的生产版本")]
        [return: ApiReturn("获取当前条码最后的生产版本")]
        public virtual double? GetWinCurrentVersion([ApiParameter("当前条码")] string barcode)
        {
        ...
        /// <returns></returns>
        [ApiService("获取当前生产资源的班次")]
        [return: ApiReturn("获取当前生产资源的班次")]
        public virtual double? GetWipResourceShift(double resourceId)
        {
            var date = RF.Find<Shift>().GetDbTime();
            var shift = RT.Service.Resolve<WipResourceController>().GetWipResourceShift(resourceId, date);
            return shift?.Id;
        }
        ...
        {
            RT.Service.Resolve<WipTemporaryRepairController>().GetRepairRecord(workOrderId, submitBarcodeCode, true);
        }
        ...
        {
            var keyItems = RT.Service.Resolve<WipTemporaryRepairController>().LoadkeyItems(barcode, isItem);
            if (keyItems.Any())
            {
        ...
        {
            RT.Service.Resolve<WipTemporaryRepairController>().Collect(barcodes, collectData, workcell);
        }
        ...
        {
            RT.Service.Resolve<WipTemporaryRepairController>().Collect(barcode, collectData, workcell, uplineProcess);
        }
        ...
        {
            return RT.Service.Resolve<WipTemporaryRepairController>().GetRoutingProcessList(barcode, workcell);
        }
        ...
        {
            return RT.Service.Resolve<WipTemporaryRepairController>().GetOptionalPaths(barcode, workcell);
        }
        ...
        {
            RT.Service.Resolve<WipTemporaryRepairController>().SaveRepairRecord(defects.AsEntityList());
        }
        ...
        {
            RT.Service.Resolve<WipTemporaryRepairController>().RepairSubmit(barcode, workcell, routingProcessId);
        }
