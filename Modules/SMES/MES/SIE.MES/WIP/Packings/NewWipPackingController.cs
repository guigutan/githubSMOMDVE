...
namespace SIE.MES.WIP.Packings
{
    ...
    /// 包装采集控制器
    /// </summary>
    public class NewWipPackingController : WipController
    {
        /// <summary>
        /// 处理包装关系
        /// </summary>
        ...
        private PackingRelation ProcessPackageRelation(string barcode, CollectData collectData, Workcell workcell)
        {
            ...
            if (currentPackingUnit == null || currentPackingUnit.IsMasterUnit)
            {
                ...
                if (itemLabel == null)
                    throw new ValidationException("未找到物料标签[{0}]".L10nFormat(barcode));
        ...
        private PackingRelation RelationPacking(Workcell workcell, AutoDoPackingMode autoDoPackingMode, PrintMode printMode, Queue<string> scannedPackageNos, double? outerPackingRelationId, WorkOrderMove workOrder, List<WorkOrderPackageRuleDetail> woRuleDtls, BatchPackingRelation innerRelation, WorkOrderPackageRuleDetail upLevelPackageRuleDetail)
        {
            ...
            if (autoDoPackingMode == AutoDoPackingMode.AutoCasePacking
                && outerPackageRelation.IsPacked
                && !outerPackageRelation.IsProcessFinish)
            {
                //包装模式为自动级联打包 且 新的外层包装已经被打包了，则进行级联打包
                return CascadePacking(workcell, workOrder, woRuleDtls, outerPackageRelation, printMode, scannedPackageNos, topRuleDtl);
            }
            else
            {
            ...
            List<WorkOrderPackageRuleDetail> woRuleDtls, ItemLabel itemLabel, WorkOrderPackageRuleDetail upLevelPackageRuleDetail)
        {
            ...
            //自动级联打包且当前包装已包装
            if (packingMode == AutoDoPackingMode.AutoCasePacking && relation.IsPacked && !relation.IsProcessFinish)
            {
                return CascadePacking(workcell, workOrder, woRuleDtls, relation, printMode, packageNos, topRuleDtl);
            }
            else
            {
                return relation;
        ...
        /// <param name="packageNos"></param>
        /// <param name="topRuleDtl">获取【工单】的当前包装工序允许打包的最顶包装</param>
        private PackingRelation CascadePacking(Workcell workcell, WorkOrderMove workOrder, List<WorkOrderPackageRuleDetail> woRuleDtls, PackingRelation relation, PrintMode printMode, Queue<string> packageNos, WorkOrderPackageRuleDetail topRuleDtl)
        {
            //当前包装关系的上一层工单包装规则明细
            var upLevelPackageRuleDetail = GetUpLevelPackingRule(woRuleDtls, workOrder.No, relation.PackageUnit);

            //级联打包，获取已打包且未关联上层包装的同一包装层级的包装关系
            var packingRelations = GetCanPackingRelations(workOrder.Id, workcell.ProcessId, workcell.StationId, relation.PackageUnitId);

            //上一层包装关系的满包数量
            int packingQty = (int)upLevelPackageRuleDetail.LevelQty;

            //当前工位存在多个没有打包的【包装单位】的包装，报错提示先手动打包完成
            if (packingRelations.Count > packingQty)
            {
                //relation 是新创建的，无法使用视图属性
                throw new ValidationException("当前工位存在超过{0}【{1}】未打包，请先手动打包。"
                    .L10nFormat(packingQty, relation.PackageUnit.Name));
            }

            if (packingRelations.Count == packingQty)
            {
                // 创建空白的上一层包装（关系）
                var upLevelPackingRelation = CreateEmptyPackingRelation(workcell, upLevelPackageRuleDetail.PackageUnit,
                    workOrder, upLevelPackageRuleDetail.LevelQty, packingRelations.Sum(p=>p.ItemQty), upLevelPackageRuleDetail.LevelQty);

                PackagingWhenFullPackage(upLevelPackingRelation, printMode, packageNos, upLevelPackageRuleDetail.NumberRule.Id, topRuleDtl);

                RF.Save(upLevelPackingRelation);

                foreach (var toBePackingRelation in packingRelations)
                {
                    toBePackingRelation.TreePId = upLevelPackingRelation.Id;
                    toBePackingRelation.RootId = upLevelPackingRelation.RootId;

                    RF.Save(toBePackingRelation);

                    //更新原来下层包装的RootId为新外包装的ID
                    DB.Update<PackingRelation>()
                        .Set(x => x.RootId, upLevelPackingRelation.Id)
                        .Where(x => x.RootId == toBePackingRelation.Id && x.Id != toBePackingRelation.Id)
                        .Execute();
                }

                //没有达到工序允许的最大包装，继续级连包装
                if (upLevelPackingRelation.IsProcessFinish)
                {
                    return upLevelPackingRelation;
                }
                else
                {
                    return CascadePacking(workcell, workOrder, woRuleDtls, upLevelPackingRelation, printMode, packageNos, topRuleDtl);
                }
            }
            else
            {
        ...
        public override ProductInfo Validate(CollectBarcode barcode, Workcell workcell)
        {
            ...
            Tuple<List<string>, Core.WorkOrders.WorkOrder, PackingUnit, Queue<PackingUnit>, bool> result
            ...
            // 要采集 包装单位 清单
            Queue<PackingUnit> toScanPackageUnits = result.Item4;
        ...
        private Tuple<List<string>, Core.WorkOrders.WorkOrder, PackingUnit, Queue<PackingUnit>, bool> ValidatePackingBarcode(
            CollectBarcode barcode, Workcell workcell)
        {
            ...
            Tuple<PackingUnit, PackingUnit, bool> packingUnits;
            ...
            //当前工序能采集的最上层的包装单位
            PackingUnit topLevelPackingUnit = packingUnits.Item2;
            ...
            //是否过站
            var needMove = packingUnits.Item3;

            return Tuple.Create(snList, workOrder, currentPackingUnint, upLevelPackageUnits, needMove);
        }
        ...
        /// <returns>当前包装单位，工序允许最大包装单位，是否需要过站</returns>
        private Tuple<PackingUnit, PackingUnit, bool> ValidateMultPackingProcess(double workOrderId, double processId, bool isSn,
            double? packageUnitId)
        {
            ...
            if (isMultPackingProcess)
            {
                ...
                return Tuple.Create(currentPackageRuleDetail.PackageUnit, maxRuleDtl.PackageUnit, needMove);
            }
            else
            {
                //上层应该包装的工单包装规则明细
                ...
                return Tuple.Create(currentPackageRuleDetail.PackageUnit, maxRuleDetail.PackageUnit, needMove);
            }
        }
        ...
        private WorkOrderPackageRuleDetail GetUpLevelPackingRule(List<WorkOrderPackageRuleDetail> woRuleDtls, string workOrderNo,
        ...
        public virtual PackingRelation PackingCollect(string barcode, CollectData collectData, Workcell workcell, bool needMove)
        {
            ...
            using (var tran = DB.TransactionScope(MesCoreEntityDataProvider.ConnectionStringName))
            {
                ...
                {
                    //需要过站
                    MoveOnPackageProcess(barcode, collectData, workcell, currentPackingUnit);
                }
        ...
        /// <param name="currentPackingUnit"></param>        
        /// <returns></returns>
        /// <exception cref="ValidationException"></exception>
        private void MoveOnPackageProcess(string barcode, CollectData collectData, Workcell workcell, PackingUnit currentPackingUnit)
        {
            if (currentPackingUnit.IsMasterUnit)
            {
                Move(new string[] { barcode }, collectData, workcell);
            }
            else
            {
                //获取包装号对应的包装关系
                var packingRelation = RT.Service.Resolve<PackingRelationController>().GetBatchPackingRelation(barcode, false);

                if (packingRelation == null)
                {
                    throw new ValidationException("采集失败,无此包装号:[{0}]".L10nFormat(barcode));
                }

                if (packingRelation.TreePId != null)
                {
                    throw new ValidationException("包装失败，{0}[{1}]已打包"
                        .L10nFormat(packingRelation.PackageUnitName, packingRelation.PackageNo));
                }
                ...
                var sns = itemLabels.Select(p => p.Label).ToArray();

                for (int i = 0; i < sns.Length; i++)
                {
                    var collectDataClone = collectData.Clone();

                    Move(new string[] { sns[i] }, collectDataClone, workcell);
                }
            }
        }
    }
