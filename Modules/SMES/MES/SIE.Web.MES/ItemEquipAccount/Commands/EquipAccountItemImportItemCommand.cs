...
namespace SIE.Web.MES.ItemEquipAccount.Commands
{
    ...
    /// 产品修改按钮
    /// </summary>
    public class EquipAccountItemImportItemCommand: ImportExcelCommand
    {
        /// <summary>
        /// 保存
        /// </summary>
        ...
        protected override void OnSave(IList<RowData> batch)
        {
            ...
            var equipAccountItems = RT.Service.Resolve<EquipAccountItemController>().GetEquipAccountItemsByprocessCodesEquipCodes(processCodes, eqCodes);
            ...
            foreach (var p in batch)
            {
                ...
                {
                    eais = equipAccountItems.Where(f => f.ProcessCode == entity.Process?.Code && f.EquipAccountCode == entity.EquipAccountCode && f.UniqueCode == entity.UniqueCode).ToList();
                    if (eais.Count > 1)
                    {
                        importResult.MessageList.Add(new ImportMessageResult() { Message = "存在多条相同工序[{0}]相同模具[{1}]相同模具组[{2}],无法识别更新哪一条数据!".L10nFormat(entity.Process?.Code, entity.EquipAccountCode, entity.UniqueCode), MsgType = ImportMessageType.SaveFail, RowNum = p.RowIndex + 1 });
                        dels.Add(p);
                        continue;
                    }
