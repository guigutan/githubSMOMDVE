...
namespace SIE.Web.MES.ItemLine.Commands
{
    ...
    /// 产线修改按钮
    /// </summary>
    public class ProductLineImportWipResourceCommand : ImportExcelCommand
    {
        /// <summary>
        /// 保存
        /// </summary>
        ...
        protected override void OnSave(IList<RowData> batch)
        {
            ...
            var productLines = RT.Service.Resolve<ProductLineController>().GetProductLinesByProcessCodesItemCodes(processCodes, itemCodes);
            ...
            foreach (var p in batch)
            {
                ...
                var pls = productLines.Where(x => x.ProcessCode == entity.Process?.Code && x.ItemCode == entity.Item?.Code).ToList();
                if (pls.Count > 1)
                {
                    importResult.MessageList.Add(new ImportMessageResult() { Message = "存在多条相同工序[{0}]相同物料[{1}],无法识别更新哪一条数据!".L10nFormat(entity.Process?.Code, entity.Item?.Code), MsgType = ImportMessageType.SaveFail, RowNum = p.RowIndex + 1 });
                    dels.Add(p);
                    continue;
                }
