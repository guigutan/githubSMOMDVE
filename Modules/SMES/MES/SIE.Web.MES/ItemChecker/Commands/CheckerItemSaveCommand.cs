...
namespace SIE.Web.MES.ItemChecker.Commands
{
    ...
    /// 工装与产品关系保存命令
    /// </summary>
    public class CheckerItemSaveCommand : SaveCommand
    {
        /// <summary>
        /// 进行保存
        /// </summary>
        ...
        protected override void DoSave(EntityList data)
        {
            ...
            if (data?.Count > 0)
            {
                ...
                {
                    if (entity is CheckerItem charac)
                    {
                        //保存时重置时间和已执行批次数量
                        ...
                        //if (RT.Service.Resolve<CheckerItemController>().GetCheckerItemBool(charac.ItemId, processId, (double)charac.CheckerUpholdId))
                        //{
