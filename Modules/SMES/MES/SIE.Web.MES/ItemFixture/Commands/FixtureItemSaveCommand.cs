...
namespace SIE.Web.MES.ItemFixture.Commands
{
    ...
    /// 工装与产品关系保存命令
    /// </summary>
    public class FixtureItemSaveCommand : SaveCommand
    {
        /// <summary>
        /// 进行保存
        /// </summary>
        ...
        protected override void DoSave(EntityList data)
        {
            ...
            if (data?.Count > 0)
            {
                ...
                {
                    if (entity is FixtureItem charac)
                    {
                        //保存时重置时间和已执行批次数量
                        ...
                        //if (RT.Service.Resolve<FixtureItemController>().GetFixtureItemBool(charac.ItemId, processId, (double)charac.FixtureUpholdId))
                        //{
