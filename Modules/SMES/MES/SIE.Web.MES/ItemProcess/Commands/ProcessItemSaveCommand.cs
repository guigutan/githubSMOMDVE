...
namespace SIE.Web.MES.ItemProcess.Commands
{
    ...
    /// 物料与工序关系保存命令
    /// </summary>
    public class ProcessItemSaveCommand : SaveCommand
    {
        /// <summary>
        /// 进行保存
        /// </summary>
        ...
        protected override void DoSave(EntityList data)
        {
            ...
            if (data?.Count > 0)
            {
                ...
                {
                    if (entity is ProcessItem charac)
                    {
                        //保存时重置时间和已执行批次数量
                        ...
                        //if (RT.Service.Resolve<ProcessItemController>().GetProcessItemBool(charac.ItemId, processId))
                        //{
