/*
 ** 批次产品工艺路线 工具栏以及画布布局JS
 * @class SIE.Web.MES.BatchRoutingDesignControl
 */
Ext.define('SIE.Web.MES.BatchRoutingDesignControl', {
    extend: 'Ext.panel.Panel',
    xtype: 'batchRoutingDesign',
    mainView: null,
    ...
    items: [{
        ...
        html: '<div style="position:absolute; width:100%; height:100%" id = "batchRoutingCanvas"></div>'
    }],
    listeners: {
        //选中节点变更事件
    ...
    initComponent: function () {
        var me = this;
        me.designCanvas = new DesignCanvas(me.mainView, 'batchRoutingCanvas', null);  //先初始化，这样外部就可以挂nodeChanged事件
        me.callParent();
    },
    ...
    updatePauseStatus: function () {
        ...
        if (layout.version && layout.batchRelation.IsPause === 0 && layout.batchRelation.IsFinish === false && layout.barcode.BatchState !== 1)   //0:No   1:Yes
            canExecute = true;
    ...
    updateSetCurrentStatus: function () {
        ...
        if (layout.version && layout.batchRelation.IsPause === 1 && selectNode) {
    ...
    updateSaveStatus: function () {
        ...
        var layout = me.mainView.layout;
        var canExecute = false;
        if (layout.version && layout.batchRelation.IsPause === 1)
            canExecute = true;
    ...
    setDesignCanvasLock: function () {
        var me = this;
        var layout = me.mainView.layout;
        if (layout.version && layout.batchRelation.IsPause === 0)   //0:No   1:Yes
    ...
    pause: function () {
        ...
        var barcode = '';
        if (layout.barcode)
            barcode = layout.barcode.BatchNo;
        SIE.Msg.askQuestion(Ext.String.format('产品：{0} 暂停，会影响产品继续生产过站，是否暂停?'.t(), barcode), function () {
            var newLayout = me.getRoutingXml();
            layout.invokeDataQuery('PauseBatchProductRouting', [barcode, me.routingLayout, newLayout], function (result) {
                //TODO刷新是异步的，需同步
    ...
    GetProcessStatus: function (dicStatus, processList, currentId, process, status, isFirst) {
        var me = this;
        //if (currentId === process.ProcessId) {
        ...
        for (var key in process.Next) {
            for (var i = 0; i < process.Next[key].length; i++) {
                ...
                var nextProcessId = nextRoutingProcess.ProcessId;
    ...
    save: function () {
        ...
        var newLayout = me.getRoutingXml();
        SIE.Msg.wait("正在保存......".t());
        layout.invokeDataQuery('SaveBatchProductRouting', [layout.version.Id, layout.batchRelation.Id, me.routingLayout, newLayout], function (result) {
            //TODO刷新是异步的，需同步
            layout.refreshWipProductInfo(layout.barcode);  //重新加载工艺路线信息
            //更新命令状态
    ...
    enabled: function () {
        ...
        if (layout.version === null || layout.version === undefined)
            throw Ext.String.format('产品:{0}还未上线'.L10N(), barcode);
        me.existCurrentProcess();
        var newLayout = me.getRoutingXml();
        SIE.Msg.wait("正在启用......".t());
        layout.invokeDataQuery('EnableBatchProductRouting', [layout.batchRelation.Id, me.routingLayout, newLayout], function (result) {
            me.refreshWipProductInfo();
            SIE.Msg.showMessage(Ext.String.format('产品：{0} 工艺路线启用成功'.L10N(), barcode));
        });
    },

    existCurrentProcess: function () {
        var me = this;
        var design = me.designCanvas.getDesignData();
        var current = design.nodes.first(function (p) { return p.designerData.ProcessState === 'Current' });
        if (current)
            return;
        var beginNode = design.nodes.first(function (p) { return p.nodeType === 'BeginNode' });
        beginNode.designerData.ProcessState = 'Current';
    },
    ...
    setNodeBoms: function () {
        ...
        for (var item in design.nodes) {
            ...
            if (processBomDic[processId]) {
                var bomList = processBomDic[processId];
                ...
                groupBoms.forEach(function (groupBom) {
                    ...
                    //throw new Error(Ext.String.format('工序[{0}]存在相同物料[{1}]的BOM'.L10N(), node.designerData.Text, bom.Code));
                });
                var processBoms = [];
                bomList.forEach(function (bom) {
                    processBoms.push({ ItemId: bom.ItemId, Qty: bom.Qty, IsBuckleMaterial: bom.IsBuckleMaterial });
                });
