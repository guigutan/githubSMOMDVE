...
namespace SIE.Wpf.MES.WIP
{
    ...
    /// 工作单元信息选择
    /// </summary>
    public static class WorkstationSelectorB
    {
        /// <summary>
        /// 选择工作单元
        /// </summary>
        ...
        public static bool SelectOperation(Workstation workstation, double? _employeeId = null)
        {
            bool isSelectSuccess = false;

            var employeeId = _employeeId ?? workstation.EmployeeId ?? RT.IdentityId;
            workstation.EmployeeId = employeeId;

            var editor = WorkCellSelectorControlFactory.CreateControl();

            if (workstation.Resources.Count > 0)
            {
                //产线列表
                editor.WipResourceList.AddRange(workstation.Resources.OrderBy(p => p.Name));
            }
            else
            {
