...
namespace SIE.Wpf.MES.WIP.PackRecombine
{
    ...
    public class PackRecombineViewModel : PackRecombineBaseViewModel
    {
        /// <summary>
        /// 条码扫完后处理逻辑
        /// </summary>
        ...
        protected override void OnBarcodeChanged(ManagedPropertyChangedEventArgs e)
        {
            ...
            try
            {
                if (PackScanMode == ScanMode.Move)
                    SplitPacking(Barcode, false);
                else if (PackScanMode == ScanMode.Join)
                    JoinPacking(Barcode, false);
                else
                    SearchPacking(Barcode, false);
            }
            catch (Exception exc)
            {
                ShowError(exc);
    ...
    /// 
    /// </summary>
    internal class PackRecombineViewModelViewConfig : WPFViewConfig<PackRecombineViewModel>
    {
