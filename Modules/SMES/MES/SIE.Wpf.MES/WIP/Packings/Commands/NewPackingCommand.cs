...
namespace SIE.Wpf.MES.WIP.Packings.Commands
{
    ...
    public class NewPackingCommand : ListViewCommand
    {
        ...
        {
            using (PerformenceWatcher.Start(logger, "NewPackingCommand"))
            {
                var vm = (view.Parent.Current as NewPackingViewModel);
                try
                {
                    if (vm == null)
                    ...
                    using (PerformenceWatcher.Start(logger, "NewPackingCommand-1"))
                    {
        ...
        private void AddCurrentPackageToView(ListLogicalView view, NewPackingViewModel vm, PackingRelation doPacked)
        {
            using (PerformenceWatcher.Start(logger, "AddCurrentPackageToView"))
            {
                if (!vm.PackingRelations.Any(e => e.PackageNo == doPacked.PackageNo))
                {
                    ...
                    {
                        var relation = vm.PackingRelations.FirstOrDefault(r => r.Id == p.Id);
                        if (relation != null)
                            vm.PackingRelations.Remove(relation);
                    }
                    vm.PackingRelations.AddRange(doPackedList);
                }
                vm.PackageRuleDetailList.MarkSaved();
            }
        }
