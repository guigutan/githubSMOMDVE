...
namespace SIE.Wpf.MES.OnOffDutyA.Commands
{
    /// <summary>
    /// 选择批次命令
    /// </summary>
    [Command(ImageName = "CheckmarkPencil", Label = "上下岗补录", ToolTip = "上下岗补录", GroupType = CommandGroupType.Edit)]
    public class OnOffDutyAInputCommand : DetailViewCommand
    {
        /// <summary>
        /// 是否可执行的逻辑
        /// </summary>
        ...
        {
            return base.CanExecute(view) && (view.Current is OnOffDutyAViewModel);
        }
        ...
        {
            var vm = view.Current as OnOffDutyAViewModel;
            ValidateWorkCell(vm);

            var template = new ListUITemplate(typeof(OnOffDutyRecrodsA), OnOffDutyARecrodsViewConfig.ListView, view.ModuleKey);// "SelectList");
            ...
            CRT.Workbench.ShowDialog(ui, w =>
              {
                  ...
                  w.Closing += (s, e) =>
                  {
                      ...
                      if (w.Result == 2)//设置上下岗时间
                      {
                          ...
                          var template1 = new DetailsUITemplate(typeof(OnOffDutyTimeSettingViewModel), OnOffDutyATimeSettingViewModeliewConfig.DetailsView, view.ModuleKey);
                              ...
                              ww.Closing += (s2, e2) =>
                              {
                                  if (ww.Result == 0)
                                  {
                                      try
                                      {
                                          ...
                                          {
                                              var selected = item as OnOffDutyRecrodsA;
                                              if (selected != null)
                                              {
                      ...
                      {
                          EntityList<OnOffDutyRecrodsA> selectedList = new EntityList<OnOffDutyRecrodsA>();
                          foreach (var item in listView.SelectedEntities)
                          {
                              var selected = item as OnOffDutyRecrodsA;
                              if (selected != null)
        ...
        void ValidateWorkCell(OnOffDutyAViewModel vm)
        {
            try
            {
               // vm.GetWorkcell();
