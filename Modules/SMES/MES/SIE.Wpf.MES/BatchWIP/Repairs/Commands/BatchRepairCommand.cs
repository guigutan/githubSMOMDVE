...
namespace SIE.Wpf.MES.BatchWIP.Repairs.Commands
{
    ...
    public class BatchRepairCommand : ListViewCommand
    {
        /// <summary>
        /// 判断维修命令能否执行
        /// </summary>
        ...
        {
            var defect = view.Current as BatchRepairDefectViewModel;
            if (defect == null) return;
            var panel = CreateControl(defect, view.ModuleKey);
            var result = CRT.Workbench.ShowDialog(Guid.NewGuid().ToString(), panel, w =>
             {
                 ...
                 w.Closing += (s, e) =>
                 {
                     if (w.Result == 0)
                     {
                         if (defect.ScrapQty < 0)
                         {
                             e.Cancel = true;
                             throw new ValidationException("报废数量不能小于0]".L10N());
                         }
                         if (defect.ScrapQty > defect.Qty)
                         {
                             e.Cancel = true;
                             throw new ValidationException("报废数量不能大于不良数量[{0}]".L10nFormat(defect.Qty));
                         }
                     }
                 };
             });
        ...
        /// <returns>控件</returns>
        private DockPanel CreateControl(BatchRepairDefectViewModel defect, string modulekey)
        {
            var panel = new DockPanel();
            var detailView = AutoUI.ViewFactory.CreateDetailView(typeof(BatchRepairDefectViewModel), BatchRepairDefectViewModelViewConfig.BatchRepairDetailView, modulekey);
        ...
        private void BindingData(BatchRepairDefectViewModel defect, MeasureControl measureEditor, ResponsibilityControl editor)
        {
