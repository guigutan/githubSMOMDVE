...
{
    /// <summary>
    /// DefectInputControl.xaml 的交互逻辑
    /// </summary>
    public partial class DefectInputControl : UserControl
    {
        #region Data
        /// <summary>
        /// 所有缺陷项目
        ...
        public static readonly DependencyProperty SelectedValueProperty =
            DependencyProperty.Register("SelectedValue", typeof(ObservableCollection<DefectItem>), typeof(DefectInputControl), new PropertyMetadata(new ObservableCollection<DefectItem>(), (s, e) => { ((DefectInputControl)s).OnValueChanged(e); }));
        ...
        public static readonly DependencyProperty DefectsProperty =
            DependencyProperty.Register("Defects", typeof(EntityList<Defect>), typeof(DefectInputControl), new PropertyMetadata(new EntityList<Defect>(), (s, e) => { ((DefectInputControl)s).OnDefectsChanged(e); }));
        ...
        public static readonly DependencyProperty PointProperty =
            DependencyProperty.Register("Point", typeof(string), typeof(DefectInputControl));
        #endregion

        /// <summary>
        /// 选中值，key：点位  value：缺陷ID
        /// </summary>
        private Dictionary<string, double> _dicSelectValue { get; } = new Dictionary<string, double>();
        ...
        protected virtual void OnDefectAdded(Defect defect)
        {
            ...
            var item = new DefectItem { Defect = defect, Control = this };
            DefectList.Add(item);
        ...
        protected virtual void OnDefectSelected(DefectItem selected)
        {
            ...
            {
                CRT.MessageService.ShowInstantMessage("请先输入点位".L10N(), "提示".L10N(), 3);
                return;
            }
        ...
        /// <param name="board">板号</param>
       private void AddSelected(DefectItem selected, string point, int? board)
        {
            DefectItem newItem = new DefectItem()
            {
            ...
            if (!AllowMultiple)
                SelectedValue.Clear();
            if (!SelectedValue.Contains(newItem))
                SelectedValue.Add(newItem);
        }
        ...
        {
            //var vm = this.DataContext as VisualInspectViewModel;
            //if (vm == null || vm.WorkOrder == null)
            //    return;
            //var pointctl = new PointInputControl();
            //Point = string.Empty;
            //pointctl.SetDefaultValue(Point);
            //int result = CRT.Workbench.ShowDialog(Guid.NewGuid().ToString(), pointctl, w =>
            //{
            //    w.Width = 680;
            //    w.Height = 400;
            //    w.MinWidth = 680;
            ...
            //        if (w.Result == 0)
            //        {
            //            var isExsit = RT.Service.Resolve<CadManageController>().IsExistPoint(vm.WorkOrder.ProductId, pointctl.Point);
            //            if (!isExsit)
            //            {
            //                arg.Cancel = true;
            //                CRT.MessageService.ShowError("点位[{0}]不存在于CAD中，请重新输入".L10nFormat(pointctl.Point));
            //            }
            //        }
            ...
            //});
            //if (result == 0)
            //    Point = pointctl.Point;
            //else
            //    Point = pointctl.InitialPoint;
        }
    }
    ...
    public class DefectItem : ObservableObject
    {
        ...
        /// Control
        /// </summary>
        public DefectInputControl Control { get; set; }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (item == null) return false;
            return Defect?.Id == item.Defect?.Id && Point == item.Point && item.BoardNo == BoardNo;
        }
