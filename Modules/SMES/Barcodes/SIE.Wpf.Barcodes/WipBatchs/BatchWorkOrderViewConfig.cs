...
namespace SIE.Wpf.Barcodes.WipBatchs
{
    ...
    public class BatchWorkOrderViewConfig : WPFViewConfig<BatchWorkOrder>
    {
        /// <summary>
        /// 批次工单视图ViewGroup
        /// </summary>
        public const string BatchWorkOrderView = "BatchWorkOrderView";
        ...
        protected void ConfigPrintView()
        {
            ...
            View.UseCommands(typeof(BatchWoGenerateCommand));
            using (View.OrderProperties())
            {
                View.Property(p => p.No).Show(ShowInWhere.All);
                View.Property(p => p.ProductCode).HasLabel("产品编码").Show(ShowInWhere.All);
                View.Property(p => p.ProductName).HasLabel("产品名称").Show(ShowInWhere.All);
                View.Property(p => p.PlanQty).HasLabel("计划数量").Show(ShowInWhere.All);
                View.Property(p => p.GeneratedQty).HasLabel("已生成批次数量").Show(ShowInWhere.All);
                View.Property(p => p.PrintedQty).HasLabel("已打印数量").Show(ShowInWhere.All);
                View.Property(p => p.PlanBeginDate).HasLabel("计划开始日期").Show(ShowInWhere.All);
