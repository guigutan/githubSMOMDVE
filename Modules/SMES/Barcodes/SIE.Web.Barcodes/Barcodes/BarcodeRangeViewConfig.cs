...
namespace SIE.Web.Barcodes
{
    ...
    public class BarcodeRangeViewConfig : WebViewConfig<BarcodeRange>
    {
        #region 条码范围
        /// <summary>
        /// 条码范围
        ...
        {
            View.UseCommands(WebCommandNames.ExportXls, "SIE.Web.Barcodes.ReceiveBarcodeCommand");
            View.Property(p => p.WONo).ShowInList(150).HasLabel("工单号");
            View.Property(p => p.ProductName).HasLabel("产品名称");
            View.Property(SnRangeProperty).ShowInList(250).HasLabel("条码范围");
            View.Property(p => p.PrintQty).UseSpinEditor(p => p.DecimalPrecision = 0);
            View.Property(p => p.ScrapedQty).UseSpinEditor(p => p.DecimalPrecision = 0);
