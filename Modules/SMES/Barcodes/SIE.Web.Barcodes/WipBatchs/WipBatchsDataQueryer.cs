...
namespace SIE.Web.Barcodes.WipBatchs
{
    ...
    public class WipBatchsDataQueryer : DataQueryer
    {
        ...
        public EntityJson GetBatchGeneratingViewModel(double workOrderId)
        {
            ...
            var wo = RF.GetById<SIE.Barcodes.WipBatchs.BatchWorkOrder>(workOrderId);
            if (wo == null)
            {
            ...
            ItemBatchRule batchRule = RT.Service.Resolve<SIE.Items.ItemController>().GetBatchRule(wo.ProductId);
            if (batchRule.BatchRule == BatchRule.WorkOrder)
            {
                var qty = RT.Service.Resolve<Core.WorkOrders.WorkOrderController>().GetWipBatch(workOrderId)?.Qty;
                if (qty == null)
                {
                    batchRule.Qty = 1;
                    warn = "工单:{0}未设置生成批次。".L10nFormat(wo.No);
                }
            }
            else if (batchRule.BatchRule == BatchRule.Vehicle)
            {
                var qty = RT.Service.Resolve<BoxController>().GetDefaultProductCapacity(wo.ProductId, null).FirstOrDefault()?.Capacity;
                if (qty == null)
                {
                    batchRule.Qty = 1;
                    warn = "物料:{0}未设置默认载具。".L10nFormat(wo.Product.Name);
                }
            }
            else
            {
            ...
            json.SetProperty("BatchRule", batchRule.BatchRule.HasValue ? ((int)batchRule.BatchRule).ToString() : string.Empty);
            json.SetProperty("BatchQty", batchRule.Qty);
